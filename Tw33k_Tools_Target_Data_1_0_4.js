...
// @version      1.0.4
// @description  A general-purpose dev/diagnostic tool for browser-based games: DOM/element inspection, storage/cookies, network traffic (catalog, diffing, filtering, waterfall), WebSocket (catalog, resend, sequences), page timing, script exec (sandboxed, run-anywhere, page/isolated/CSP-proof path-access modes), JS console, Token Inspector, Recorder, Event/DOM/Storage watchers, Snapshots, Value Tracer, Replay/Edit (sweeps, automation, privileged send), page-load traffic diffing, Export/AI Briefing.
// @author       Tw33k
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    const PageInspectorUI = {
        ...
        _exportCategories: { traffic: true, catalog: true, recorder: true, element: true, pageTiming: true, websocket: true, traceHistory: true, replay: true, replayHistory: true, sandboxRun: true, domSnapshot: true, eventDebugLog: true, domMutationLog: true, storageWatchLog: true, wsMessageCatalog: true, snapshots: true },
        ...
        _renderExport() {
            ...
            ['traffic', 'catalog', 'recorder', 'element', 'pageTiming', 'websocket', 'wsMessageCatalog', 'traceHistory', 'replay', 'replayHistory', 'sandboxRun', 'domSnapshot', 'eventDebugLog', 'domMutationLog', 'storageWatchLog', 'snapshots'].forEach((key) => {
        ...
        _updateEndpointsList() {
            ...
            if (countEl) countEl.textContent = `${all.length} distinct endpoints observed - persists across page loads, builds up the more you browse.`;
        ...
        show() {
            ...
            panel.innerHTML = `
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <button id="ttd-settings-clear-captured" style="${Helpers._secondaryBtnStyle(t)}width:100%;margin-bottom:6px;">Clear traffic + recorder + snapshots (${ObservedTraffic.all().length + InvestigationRecorder.timeline().length + SnapshotManager.all().length})</button>
            ...
            document.getElementById('ttd-settings-clear-captured').onclick = (e) => {
                if (!confirm('Clear captured Traffic history, the Recorder timeline, and saved Snapshots? This does not touch the page\'s real localStorage, sessionStorage, or cookies.')) return;
                ObservedTraffic.clearEntries();
                InvestigationRecorder.clear();
                SnapshotManager.clear();
                e.target.textContent = 'Cleared!';
                setTimeout(() => { e.target.textContent = 'Clear traffic + recorder + snapshots (0)'; }, 1200);
            };
