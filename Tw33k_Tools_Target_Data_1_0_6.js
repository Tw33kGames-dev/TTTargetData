...
(function () {
    ...
    const InvestigationRecorder = {
        ...
        timeline() {
            ...
            const combined = [...this._entries, ...networkEntries];
    ...
    const InvestigationExport = {
        ...
        toCSV(categories) {
            ...
            if (categories.persistedTraffic && data.persistedTraffic && data.persistedTraffic.length) {
                const rows = [this._csvRow(['Timestamp', 'Method', 'Host', 'PathPattern', 'Status', 'RequestBody', 'ResponseBody'])];
                data.persistedTraffic.forEach((e) => {
                    rows.push(this._csvRow([new Date(e.timestamp).toISOString(), e.method, e.host, e.pathPattern, e.status ?? '', e.requestBody || '', e.responseBody || '']));
                });
        ...
        briefing(categories) {
            ...
            if (data.persistedTraffic && data.persistedTraffic.length) {
                lines.push(`PERSISTED TRAFFIC CACHE (survives reload, up to ${ObservedTraffic.PERSISTED_ENTRIES_PER_KEY} distinct requests kept per endpoint pattern, ${data.persistedTraffic.length} total entr${data.persistedTraffic.length === 1 ? 'y' : 'ies'}):`);
                data.persistedTraffic.forEach((e) => {
                    lines.push(`- ${e.method} ${e.host}${e.pathPattern} - status ${e.status}, last seen ${new Date(e.timestamp).toLocaleString()}`);
    ...
    const WebSocketMonitor = {
        ...
        all() {
            return this._connections.slice().reverse();
        },
        ...
        catalog() {
            ...
            this._connections.forEach((conn) => {
                conn.messages.forEach((m) => {
                    ...
                    if (!g) {
                        g = { url: conn.url, direction: m.direction, shape, count: 0, lastSeen: 0, sample: m.text.length > 200 ? m.text.slice(0, 200) + '...' : m.text };
                        groups.set(key, g);
                    }
                    g.count++;
    ...
    const ObservedTraffic = {
        ...
        // Each catalog key keeps a small ring of *distinct* request bodies, not just the
        // latest one - a multi-step flow (e.g. a trade: start, load your items, load their
        // items, confirm, accept) fires several different POST bodies against the same
        // pathPattern across several page refreshes, and a single overwritten slot would
        // only ever show the last step. Repeats of an identical body (e.g. polling GETs)
        // just refresh the timestamp on the existing item instead of growing the ring.
        PERSISTED_ENTRIES_PER_KEY: 5,
        ...
        record(entry) {
            ...
            const catalogKey = `${entry.method} ${host}${pattern}`;
            ...
            catEntry.statusesObserved.add(entry.status);
            ...
            this._updatePersistedTraffic(catalogKey, entry, host, pattern);
        ...
        // Keeps up to PERSISTED_ENTRIES_PER_KEY distinct request bodies per catalog key, so a
        ...
        // instead of the last one clobbering the rest. An identical repeat (e.g. polling
        // GETs with no body) just refreshes the timestamp on the matching existing item
        // rather than growing the ring - only genuinely distinct bodies take a new slot.
        _updatePersistedTraffic(catalogKey, entry, host, pattern) {
            ...
            const dupeIndex = ring.findIndex((e) => (e.requestBody || null) === (requestBody || null));

            if (dupeIndex !== -1) {
                // Same body seen before for this key - refresh it in place (moves it to the
        ...
        hostSummary() {
            ...
            this._entries.forEach((entry) => {
                let host;
                try { host = new URL(entry.url).host; } catch { host = '(unparseable URL)'; }
    ...
    const PageInspectorUI = {
        ...
        _exportCategories: { traffic: true, catalog: true, persistedTraffic: true, recorder: true, element: true, pageTiming: true, websocket: true, traceHistory: true, replay: true, replayHistory: true, sandboxRun: true, domSnapshot: true, eventDebugLog: true, domMutationLog: true, storageWatchLog: true, wsMessageCatalog: true, snapshots: true },
        ...
        _renderElement() {
            ...
            area.innerHTML = `
                ...
                <button id="ttd-pi-element-pick" style="${Helpers._secondaryBtnStyle(t)}width:100%;">${info ? 'Pick a different element' : 'Pick element'}</button>
                <div id="ttd-pi-element-detail" style="margin-top:8px;"></div>
            `;

            document.getElementById('ttd-pi-element-pick').onclick = () => {
                UI.enterPickMode((el) => {
                    if (el) this._pickedElement = el;
                });
            };
        ...
        _renderRecorder() {
            ...
            timeline.slice(-300).forEach((entry) => {
                ...
                const clickable = entry.kind === 'network';
        ...
        _timelineKindLabel(entry) {
            ...
            if (entry.kind === 'network') return `${entry.method} ${Helpers._shortenUrl(entry.url)} -> ${entry.status ?? '?'} (${entry.durationMs}ms)`;
        ...
        _renderExport() {
            ...
            area.innerHTML = `
                ...
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:8px;">Persisted traffic cache: ${persistedTrafficCount} total entr${persistedTrafficCount === 1 ? 'y' : 'ies'} (up to ${ObservedTraffic.PERSISTED_ENTRIES_PER_KEY} distinct requests kept per endpoint pattern), ~${(persistedTrafficBytes / 1024).toFixed(1)}KB of ${(Config.persistedTrafficBudgetBytes / 1024).toFixed(0)}KB budget - survives page reloads without needing Traffic history to stay open.</div>
                ${persistedTrafficWarning}
                <div style="margin-bottom:8px;">
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${trafficCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-traffic" ${cats.traffic ? 'checked' : ''} ${trafficCount ? '' : 'disabled'}> Network requests (${trafficCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${catalogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-catalog" ${cats.catalog ? 'checked' : ''} ${catalogCount ? '' : 'disabled'}> Endpoint catalog (${catalogCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${persistedTrafficCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-persistedTraffic" ${cats.persistedTraffic ? 'checked' : ''} ${persistedTrafficCount ? '' : 'disabled'}> Persisted traffic cache - survives reload (${persistedTrafficCount} entries)</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${recorderCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-recorder" ${cats.recorder ? 'checked' : ''} ${recorderCount ? '' : 'disabled'}> Recorder timeline (${recorderCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${hasElement ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-element" ${cats.element ? 'checked' : ''} ${hasElement ? '' : 'disabled'}> Last picked element${hasElement ? '' : ' (none picked yet)'}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${hasPageTiming ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-pageTiming" ${cats.pageTiming ? 'checked' : ''} ${hasPageTiming ? '' : 'disabled'}> Page load timing${hasPageTiming ? '' : ' (unavailable)'}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${websocketCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-websocket" ${cats.websocket ? 'checked' : ''} ${websocketCount ? '' : 'disabled'}> WebSocket activity (${websocketCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${wsMessageCatalogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-wsMessageCatalog" ${cats.wsMessageCatalog ? 'checked' : ''} ${wsMessageCatalogCount ? '' : 'disabled'}> WebSocket message catalog (${wsMessageCatalogCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${traceHistoryCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-traceHistory" ${cats.traceHistory ? 'checked' : ''} ${traceHistoryCount ? '' : 'disabled'}> Value Tracer history (${traceHistoryCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${hasReplay ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-replay" ${cats.replay ? 'checked' : ''} ${hasReplay ? '' : 'disabled'}> Last Replay result${hasReplay ? '' : ' (none sent yet)'}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${replayHistoryCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-replayHistory" ${cats.replayHistory ? 'checked' : ''} ${replayHistoryCount ? '' : 'disabled'}> Replay session history (${replayHistoryCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${hasSandboxRun ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-sandboxRun" ${cats.sandboxRun ? 'checked' : ''} ${hasSandboxRun ? '' : 'disabled'}> Last Sandbox run${hasSandboxRun ? '' : ' (none run yet)'}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${domSnapshotCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-domSnapshot" ${cats.domSnapshot ? 'checked' : ''} ${domSnapshotCount ? '' : 'disabled'}> DOM snapshot - storage/cookies (${domSnapshotCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${eventDebugLogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-eventDebugLog" ${cats.eventDebugLog ? 'checked' : ''} ${eventDebugLogCount ? '' : 'disabled'}> Event Debugger log (${eventDebugLogCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${domMutationLogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-domMutationLog" ${cats.domMutationLog ? 'checked' : ''} ${domMutationLogCount ? '' : 'disabled'}> DOM Mutation Watcher log (${domMutationLogCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${storageWatchLogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-storageWatchLog" ${cats.storageWatchLog ? 'checked' : ''} ${storageWatchLogCount ? '' : 'disabled'}> Storage/Cookie Watcher log (${storageWatchLogCount})</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${snapshotsCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-snapshots" ${cats.snapshots ? 'checked' : ''} ${snapshotsCount ? '' : 'disabled'}> Snapshots (${snapshotsCount})</label>
                </div>
                <div style="display:flex;gap:6px;margin-bottom:6px;">
            ...
            ['traffic', 'catalog', 'persistedTraffic', 'recorder', 'element', 'pageTiming', 'websocket', 'wsMessageCatalog', 'traceHistory', 'replay', 'replayHistory', 'sandboxRun', 'domSnapshot', 'eventDebugLog', 'domMutationLog', 'storageWatchLog', 'snapshots'].forEach((key) => {
        ...
        _renderStorage() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-content');
            const entries = PageInspector.getStorageDump(this._storageType === 'local' ? localStorage : sessionStorage);
            ...
            area.innerHTML = `
                ...
                <div id="ttd-pi-storage-list" style="max-height:300px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;"></div>
            `;

            document.getElementById('ttd-pi-storage-local').onclick = () => { this._storageType = 'local'; this._renderStorage(); };
            document.getElementById('ttd-pi-storage-session').onclick = () => { this._storageType = 'session'; this._renderStorage(); };
            ...
            entries.forEach((en) => {
                const row = document.createElement('div');
                row.style.cssText = `padding:6px 8px;font-size:11px;border-bottom:1px solid ${t.rowBorder};cursor:pointer;word-break:break-all;`;
                const preview = en.value.length > 150 ? en.value.slice(0, 150) + '...' : en.value;
                ...
                row.onclick = async () => { await copyToClipboard(en.value); };
                listEl.appendChild(row);
            });
        },
        ...
        _renderCookies() {
            ...
            area.innerHTML = `
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:6px;">Only cookies readable from JS - httpOnly session cookies won't appear here.</div>
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                ...
                <div id="ttd-pi-cookies-list" style="max-height:300px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;"></div>
            `;
            ...
            cookies.forEach((c) => {
                ...
                row.onclick = async () => { await copyToClipboard(c.value); };
                listEl.appendChild(row);
            });
        },
        ...
        _updateTrafficList() {
            ...
            if (countEl) countEl.textContent = `${all.length} of up to ${ObservedTraffic.MAX_ENTRIES} kept, in-memory only (cleared on page reload) - excludes this tool's own Explorer/Console/Catalog calls.`;
            ...
            filtered.slice(0, 200).forEach((entry) => {
                ...
                const statusColor = entry.status >= 200 && entry.status < 300 ? t.statusOk : t.statusBad;
                const row = document.createElement('div');
                row.style.cssText = `padding:6px 8px;font-size:11px;cursor:pointer;border-bottom:1px solid ${t.rowBorder};background:${isSelected ? t.secondaryBtnBg : 'transparent'};word-break:break-all;`;
                ...
                const reasonLine = reasons && reasons.length
                    ...
                    : '';
                row.innerHTML = `<span style="font-weight:700;">${Helpers._escape(entry.method)}</span> <span style="color:${statusColor};">${entry.status ?? '?'}</span> ${Helpers._escape(Helpers._shortenUrl(entry.url))}${recordedBadge} <span style="color:${t.cardDesc};">- ${Helpers._timeAgo(entry.timestamp)}</span>${reasonLine}`;
        ...
        _renderTrafficDetail() {
            ...
            el.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    <span style="font-size:11px;color:${statusColor};font-weight:700;">${entry.method} ${entry.status ?? '?'} - ${entry.durationMs}ms - ${entry.size?.toLocaleString() ?? '?'} bytes</span>
                    <div style="display:flex;gap:6px;">
        ...
        _renderPersistedTrafficSection(entries, t) {
            ...
            const items = entries.map((e, i) => {
                ...
                return `
                    <div style="font-size:10px;color:${t.cardDesc};margin-top:${i === 0 ? '2' : '10'}px;margin-bottom:2px;">${i === 0 ? 'Most recent' : `${i + 1} back`} - ${Helpers._timeAgo(e.timestamp)} - status ${e.status ?? 'unknown'}</div>
                    ${bodyBlock}
                `;
            }).join('');
            return `
                <div style="font-size:11px;font-weight:700;margin-top:8px;margin-bottom:2px;">Persisted traffic cache (survives reload) - ${entries.length} distinct request${entries.length === 1 ? '' : 's'} kept for this endpoint</div>
                ${items}
            `;
        },
