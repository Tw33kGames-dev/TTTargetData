...
(function () {
    ...
    const InvestigationExport = {
        ...
        briefing(categories) {
            ...
            if (data.persistedTraffic && data.persistedTraffic.length) {
                lines.push(`PERSISTED TRAFFIC CACHE (survives reload, one entry per endpoint pattern, ${data.persistedTraffic.length} pattern${data.persistedTraffic.length === 1 ? '' : 's'}):`);
                data.persistedTraffic.forEach((e) => {
    ...
    const ObservedTraffic = {
        ...
        // deliberately small: it's what actually crosses into GM_setValue, one entry per
        // catalog key (method+host+pathPattern), so it stays cheap regardless of how many
        // times that endpoint gets hit.
        ...
        _loadPersistedTraffic() {
            ...
            if (stored && typeof stored === 'object') this._persistedTraffic = stored;
        },
        ...
        // One lean entry per catalog key (latest hit wins), so a chatty low-value endpoint
        // (page.php polling, itemmarket, etc.) can never push a rare high-value one (trade.php)
        // out of the persisted cache the way a flat rolling buffer would.
        _updatePersistedTraffic(catalogKey, entry, host, pattern) {
            const trunc = this.PERSISTED_BODY_TRUNC;
            this._persistedTraffic[catalogKey] = {
                method: entry.method,
                host,
                pathPattern: pattern,
                url: entry.url,
                ...
                requestBody: this._truncateForPersist(entry.requestBody, trunc),
                responseBody: this._truncateForPersist(
        ...
        // Enforces a soft byte budget by dropping the least-recently-updated catalog keys
        // first, re-measuring after each drop. O(n^2)-ish but n is one entry per distinct
        // endpoint pattern (tens, not hundreds), so this stays cheap in practice.
        _enforceByteBudget(map, budgetBytes) {
            const working = { ...map };
            const keys = Object.keys(working).sort((a, b) => (working[a].timestamp || 0) - (working[b].timestamp || 0));
            let size;
            try { size = JSON.stringify(working).length; } catch { size = 0; }
            while (keys.length && size > budgetBytes) {
                delete working[keys.shift()];
                try { size = JSON.stringify(working).length; } catch { size = 0; }
            }
            return working;
        },
        ...
        persistedTrafficEntries() {
            return Object.keys(this._persistedTraffic)
                .map((k) => ({ catalogKey: k, ...this._persistedTraffic[k] }))
    ...
    const PageInspectorUI = {
        ...
        _renderExport() {
            ...
            area.innerHTML = `
                ...
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:8px;">Persisted traffic cache: ${persistedTrafficCount} endpoint pattern${persistedTrafficCount === 1 ? '' : 's'}, ~${(persistedTrafficBytes / 1024).toFixed(1)}KB of ${(Config.persistedTrafficBudgetBytes / 1024).toFixed(0)}KB budget - one entry per pattern, so it survives page reloads without needing Traffic history to stay open.</div>
                ...
                <div style="margin-bottom:8px;">
                    ...
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${persistedTrafficCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-persistedTraffic" ${cats.persistedTraffic ? 'checked' : ''} ${persistedTrafficCount ? '' : 'disabled'}> Persisted traffic cache - survives reload (${persistedTrafficCount})</label>
