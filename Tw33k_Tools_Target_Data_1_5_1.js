...
(function () {
    ...
    const Config = {
        ...
        get snapshotsPersistBudgetBytes() {
            const v = Storage.get('ttd_snapshots_budget', 150000);
            const n = Number(v);
            return Number.isFinite(n) && n > 0 ? Math.floor(n) : 150000;
        },
        set snapshotsPersistBudgetBytes(value) {
            const n = Number(value);
            Storage.set('ttd_snapshots_budget', Number.isFinite(n) && n > 0 ? Math.floor(n) : 150000);
        },

        get replayHistoryBudgetBytes() {
            const v = Storage.get('ttd_replay_history_budget', 100000);
            const n = Number(v);
            return Number.isFinite(n) && n > 0 ? Math.floor(n) : 100000;
        },
        set replayHistoryBudgetBytes(value) {
            const n = Number(value);
            Storage.set('ttd_replay_history_budget', Number.isFinite(n) && n > 0 ? Math.floor(n) : 100000);
        },
    ...
    const StorageWatcher = {
        ...
        WATCHES_KEY: 'ttd_storage_watches',
        LOG_KEY: 'ttd_storage_watch_log',
        ...
        install() {
            this._loadLog();
            const stored = Storage.get(this.WATCHES_KEY, null);
            if (Array.isArray(stored)) {
                stored.forEach((w) => { if (w && w.kind && w.key) this.addWatch(w.kind, w.key); });
            }
            const flush = () => {
                if (this._saveLogTimer) { clearTimeout(this._saveLogTimer); this._saveLogTimer = null; }
                this._persistLog();
            };
            window.addEventListener('pagehide', flush);
            window.addEventListener('beforeunload', flush);
        ...
        _saveWatches() {
            Storage.set(this.WATCHES_KEY, this._watches.map((w) => ({ kind: w.kind, key: w.key })));
        },
    ...
    const SnapshotManager = {
        ...
        install() {
            const stored = Storage.get(this.KEY, null);
            if (Array.isArray(stored)) {
                this._snapshots = stored;
                this._nextId = this._snapshots.reduce((max, s) => Math.max(max, s.id || 0), 0) + 1;
            }
            const flush = () => {
                if (this._saveTimer) { clearTimeout(this._saveTimer); this._saveTimer = null; }
                this._persistNow();
        ...
        _persistNow() {
            const budget = Config.snapshotsPersistBudgetBytes;
            const snaps = this._snapshots.slice();
            const measure = () => { try { return JSON.stringify(snaps).length; } catch { return 0; } };
            while (snaps.length && measure() > budget) snaps.shift();
            Storage.set(this.KEY, snaps);
        },
        ...
        restore(id) {
            ...
            let count = 0;
            const errors = [];
            Object.keys(snap.data.localStorage || {}).forEach((key) => {
                const r = PageInspector.setStorageItem(localStorage, key, snap.data.localStorage[key]);
                if (r.ok) count++; else errors.push(`localStorage.${key}: ${r.error}`);
            });
            Object.keys(snap.data.sessionStorage || {}).forEach((key) => {
                const r = PageInspector.setStorageItem(sessionStorage, key, snap.data.sessionStorage[key]);
                if (r.ok) count++; else errors.push(`sessionStorage.${key}: ${r.error}`);
            });
            Object.keys(snap.data.cookies || {}).forEach((key) => {
                const r = PageInspector.setCookie(key, snap.data.cookies[key]);
                if (r.ok) count++; else errors.push(`cookie.${key}: ${r.error}`);
            });
    ...
    const TokenVault = {
        ...
        install() {
            ...
            if (Array.isArray(stored)) {
                ...
                this._nextId = this._entries.reduce((max, e) => Math.max(max, e.id || 0), 0) + 1;
            }
        },

        _save() {
            Storage.set(this.KEY, this._entries);
        },
        ...
        addManual(value, label) {
            return this.add({ source: 'manual', location: 'manually entered', value: String(value || '').trim() }, label);
        },

        updateValue(id, newValue) {
            const entry = this._entries.find((e) => e.id === id);
            if (!entry) return null;
            entry.value = String(newValue || '').trim();
    ...
    const StorageVault = {
        ...
        apply(id, overrideKey) {
            const entry = this._entries.find((e) => e.id === id);
            if (!entry) return { ok: false, error: 'Vault entry not found.' };
            const key = (overrideKey || entry.key || '').trim();
            if (!key) return { ok: false, error: 'Key required.' };
            if (entry.kind === 'local') return PageInspector.setStorageItem(localStorage, key, entry.value);
            if (entry.kind === 'session') return PageInspector.setStorageItem(sessionStorage, key, entry.value);
            if (entry.kind === 'cookie') return PageInspector.setCookie(key, entry.value);
            return { ok: false, error: `Unknown kind "${entry.kind}".` };
        }
    };

    const InvestigationExport = {
        async gather(categories) {
            const data = {};
            ...
            if (categories.domSnapshot) {
                ...
                const idb = await IndexedDBInspector.snapshotAllRows(50, 500);
                data.domSnapshot = [...local, ...session, ...cookies, ...idb];
            }
            ...
            if (categories.tokenVault) data.tokenVault = TokenVault.all();
            if (categories.storageVault) data.storageVault = StorageVault.all();
        ...
        async rawExport(categories) {
            return JSON.stringify(await this.gather(categories), null, 2);
        },
        ...
        async toCSV(categories) {
            const data = await this.gather(categories);
            const tables = [];
            ...
            if (categories.catalog && data.catalog && data.catalog.length) {
                const rows = [this._csvRow(['Method', 'Host', 'PathPattern', 'CallCount', 'FirstSeen', 'LastSeen', 'Params', 'Statuses', 'ContentTypes', 'ExampleUrl', 'ExampleResponsePreview'])];
                data.catalog.forEach((c) => {
                    rows.push(this._csvRow([
                        c.method, c.host, c.pathPattern, c.callCount,
                        new Date(c.firstSeen).toISOString(), new Date(c.lastSeen).toISOString(),
                        (c.paramsObserved || []).join('|'), (c.statusesObserved || []).join('|'), (c.contentTypesObserved || []).join('|'),
                        c.exampleUrl || '', c.exampleResponsePreview || ''
                    ]));
                });
                tables.push(`## Endpoint catalog (${data.catalog.length})\n${rows.join('\n')}`);
            }

            if (categories.recorder && data.recorderTimeline && data.recorderTimeline.length) {
                const rows = [this._csvRow(['Timestamp', 'Kind', 'Detail'])];
                data.recorderTimeline.forEach((e) => {
                    const rest = Object.keys(e).filter((k) => k !== 'timestamp' && k !== 'kind')
                        .map((k) => `${k}=${typeof e[k] === 'object' ? JSON.stringify(e[k]) : e[k]}`)
                        .join('; ');
                    rows.push(this._csvRow([new Date(e.timestamp).toISOString(), e.kind, rest]));
                });
                tables.push(`## Recorder timeline (${data.recorderTimeline.length})\n${rows.join('\n')}`);
            }

            if (categories.websocket && data.websocket && data.websocket.length) {
                const rows = [this._csvRow(['ConnectionUrl', 'ConnectionStatus', 'OpenedAt', 'ClosedAt', 'Direction', 'MessageTimestamp', 'MessageText', 'MessageSize'])];
                let messageCount = 0;
                data.websocket.forEach((conn) => {
                    const opened = conn.openedAt ? new Date(conn.openedAt).toISOString() : '';
                    const closed = conn.closedAt ? new Date(conn.closedAt).toISOString() : '';
                    if (!conn.messages || !conn.messages.length) {
                        rows.push(this._csvRow([conn.url, conn.status || '', opened, closed, '', '', '', '']));
                        return;
                    }
                    conn.messages.forEach((m) => {
                        messageCount++;
                        const text = m.text && m.text.length > 500 ? m.text.slice(0, 500) + '...' : (m.text || '');
                        rows.push(this._csvRow([conn.url, conn.status || '', opened, closed, m.direction, new Date(m.timestamp).toISOString(), text, m.size ?? '']));
                    });
                });
                tables.push(`## WebSocket activity (${data.websocket.length} connection${data.websocket.length === 1 ? '' : 's'}, ${messageCount} message${messageCount === 1 ? '' : 's'})\n${rows.join('\n')}`);
            }

            if (categories.snapshots && data.snapshots && data.snapshots.length) {
                const rows = [this._csvRow(['Label', 'Timestamp', 'Url', 'LocalStorageCount', 'SessionStorageCount', 'CookiesCount', 'GlobalsCount'])];
                data.snapshots.forEach((s) => {
                    rows.push(this._csvRow([
                        s.label, new Date(s.timestamp).toISOString(), s.url,
                        Object.keys(s.data.localStorage || {}).length, Object.keys(s.data.sessionStorage || {}).length,
                        Object.keys(s.data.cookies || {}).length, Object.keys(s.data.globals || {}).length
                    ]));
                });
                tables.push(`## Snapshots (${data.snapshots.length})\n${rows.join('\n')}`);
            }
            ...
            if (categories.replayHistory && data.replayHistory && data.replayHistory.length) {
                const rows = [this._csvRow(['Timestamp', 'Method', 'Url', 'RequestBody', 'Ok', 'Status', 'DurationMs', 'ResponseBody', 'Error', 'SweepValue', 'Anomaly', 'AnomalyReasons'])];
                data.replayHistory.forEach((e) => {
                    const r = e.result || {};
                    let responseBody = '';
                    try { responseBody = (r.json !== undefined && r.json !== null) ? JSON.stringify(r.json) : (r.rawText || ''); } catch { responseBody = r.rawText || ''; }
                    if (responseBody.length > 500) responseBody = responseBody.slice(0, 500) + '...';
                    rows.push(this._csvRow([
                        new Date(e.timestamp).toISOString(), e.draft.method, e.draft.url, e.draft.body || '',
                        r.ok ? 'true' : 'false', r.status ?? '', r.durationMs ?? '', responseBody, r.error || '',
                        e.sweep ? String(e.sweep.value) : '', e.sweep ? (e.sweep.anomaly ? 'true' : 'false') : '',
                        e.sweep && e.sweep.anomalyReasons ? e.sweep.anomalyReasons.join('; ') : ''
                    ]));
                });
                tables.push(`## Replay history (${data.replayHistory.length})\n${rows.join('\n')}`);
            }

            if (categories.tokenVault && data.tokenVault && data.tokenVault.length) {
                const rows = [this._csvRow(['Label', 'Source', 'Location', 'SavedAt', 'Value'])];
                data.tokenVault.forEach((tk) => {
                    rows.push(this._csvRow([tk.label || '', tk.source || '', tk.location || '', new Date(tk.savedAt).toISOString(), tk.value]));
                });
                tables.push(`## Token Vault (${data.tokenVault.length}) - contains raw sensitive token values\n${rows.join('\n')}`);
            }

            if (categories.storageVault && data.storageVault && data.storageVault.length) {
                const rows = [this._csvRow(['Kind', 'Key', 'Label', 'SavedAt', 'Value'])];
                data.storageVault.forEach((sv) => {
                    rows.push(this._csvRow([sv.kind, sv.key, sv.label || '', new Date(sv.savedAt).toISOString(), sv.value]));
                });
                tables.push(`## Storage/Cookie Vault (${data.storageVault.length})\n${rows.join('\n')}`);
            }
            ...
            if (categories.misconfig && data.misconfig && data.misconfig.length) {
                const rows = [this._csvRow(['Severity', 'Label', 'Detail'])];
                data.misconfig.forEach((f) => {
            ...
            if (categories.findings && data.findings && data.findings.length) {
                const rows = [this._csvRow(['Score', 'Confidence', 'Source', 'Label', 'Detail'])];
                data.findings.forEach((f) => {
                    rows.push(this._csvRow([f.score, f.confidence, f.source, f.label, f.detail]));
                });
                tables.push(`## Findings, consolidated (${data.findings.length})\n${rows.join('\n')}`);
            }
        ...
        async briefing(categories) {
            ...
            if (data.replayHistory && data.replayHistory.length) {
                ...
                const flaggedCount = chronological.filter((e) => e.sweep && e.sweep.anomaly).length;
                lines.push(`REQUEST REPLAY SESSION HISTORY: ${chronological.length} send${chronological.length === 1 ? '' : 's'} this session${flaggedCount ? `, ${flaggedCount} sweep result${flaggedCount === 1 ? '' : 's'} flagged as anomalous` : ''}:`);
                chronological.slice(0, 15).forEach((entry, i) => {
                    ...
                    const sweepNote = entry.sweep ? ` [sweep value: ${entry.sweep.value}${entry.sweep.anomaly ? `, FLAGGED: ${entry.sweep.anomalyReasons.join('; ')}` : entry.sweep.isBaseline ? ', baseline' : ''}]` : '';
                    lines.push(`${i + 1}. ${entry.draft.method} ${entry.draft.url} -> ${status}${sweepNote}`);
                });
            ...
            if (data.domSnapshot && data.domSnapshot.length) {
                ...
                const sourceLabels = Object.keys(bySource).map((s) => (s.startsWith('indexeddb:') ? 'IndexedDB' : s));
                const sourceSummary = Array.from(new Set(sourceLabels)).join('/');
                lines.push(`DOM SNAPSHOT: ${data.domSnapshot.length} key${data.domSnapshot.length === 1 ? '' : 's'} across ${sourceSummary}:`);
            ...
            if (data.tokenVault && data.tokenVault.length) {
                lines.push(`TOKEN VAULT: ${data.tokenVault.length} saved - RAW TOKEN VALUES, treat this section as sensitive:`);
                data.tokenVault.forEach((tk) => {
                    lines.push(`- ${tk.label || '(unlabeled)'} [${tk.source}] ${tk.location} - saved ${new Date(tk.savedAt).toLocaleString()}`);
                    lines.push(`    ${tk.value}`);
                });
                lines.push('');
            }

            if (data.storageVault && data.storageVault.length) {
                lines.push(`STORAGE/COOKIE VAULT: ${data.storageVault.length} saved:`);
                data.storageVault.forEach((sv) => {
                    lines.push(`- ${sv.kind}:${sv.key}${sv.label ? ` (${sv.label})` : ''} - saved ${new Date(sv.savedAt).toLocaleString()}`);
                    lines.push(`    ${sv.value}`);
                });
                lines.push('');
            }
            ...
            if (data.misconfig && data.misconfig.length) {
                lines.push(`MISCONFIG AUDIT: ${data.misconfig.length} finding${data.misconfig.length === 1 ? '' : 's'}:`);
                data.misconfig.forEach((f) => {
    ...
    const RequestReplay = {
        ...
        PAYLOAD_PRESETS: {
            sqli: {
                ...
                values: [
                    ...
                    '1 OR 1=1', "'; DROP TABLE users; --", "' UNION SELECT NULL-- ",
                    '1 AND SLEEP(5)', "' AND SLEEP(5)-- ", '1) OR (1=1', "admin'--"
                ]
            },
            xss: {
                label: 'XSS probes',
                values: [
                    ...
                    'javascript:alert(1)', '<svg onload=alert(1)>', '"><svg/onload=alert(1)>',
                    '\'"--></style></script><script>alert(1)</script>', '<body onload=alert(1)>'
                ]
            },
            pathTraversal: {
                ...
                values: [
                    ...
                    '/etc/passwd', '..%2f..%2f..%2fetc%2fpasswd', 'file:///etc/passwd'
                ]
            }
        },
        ...
        detectAnomalies(log) {
            const baseline = log.find((l) => l.ok) || null;
            return log.map((l) => {
                const reasons = [];
                if (baseline && l !== baseline) {
                    if (l.ok) {
                        if (l.status !== baseline.status) reasons.push({ type: 'status', text: `status ${l.status} vs baseline ${baseline.status}` });
                        if (typeof baseline.length === 'number' && typeof l.length === 'number' && baseline.length > 0) {
                            const deltaPct = Math.abs(l.length - baseline.length) / baseline.length;
                            if (deltaPct >= this.ANOMALY_THRESHOLDS.lengthDeltaPct) {
                                reasons.push({ type: 'length', text: `body length ${l.length}B vs baseline ${baseline.length}B (${Math.round(deltaPct * 100)}% delta)` });
                            }
                        }
                        if (typeof baseline.durationMs === 'number' && typeof l.durationMs === 'number' && baseline.durationMs > 0) {
                            const ratio = l.durationMs / baseline.durationMs;
                            const absDelta = l.durationMs - baseline.durationMs;
                            if (ratio >= this.ANOMALY_THRESHOLDS.timingRatio && absDelta >= this.ANOMALY_THRESHOLDS.timingAbsMs) {
                                reasons.push({ type: 'timing', text: `${l.durationMs}ms vs baseline ${baseline.durationMs}ms (${ratio.toFixed(1)}x slower - possible time-based behavior)` });
                            }
                        }
                    } else if (l.error) {
                        reasons.push({ type: 'failed', text: `request failed: ${l.error}` });
                    }
                }
                if (l.errorSignatures && l.errorSignatures.length) {
                    l.errorSignatures.forEach((sig) => reasons.push({ type: 'error-signature', text: `error signature: ${sig}` }));
                }
                return Object.assign({}, l, {
                    anomaly: reasons.length > 0,
                    anomalyReasons: reasons.map((r) => r.text),
                    anomalyReasonTypes: reasons.map((r) => r.type),
                    isBaseline: l === baseline
                });
            });
        },
    ...
    const ReplayHistory = {
        ...
        install() {
            const stored = Storage.get(this.KEY, null);
            if (Array.isArray(stored)) this._entries = stored;
            this._idCounter = this._entries.reduce((max, e) => Math.max(max, e.id || 0), 0);
            const flush = () => {
                if (this._saveTimer) { clearTimeout(this._saveTimer); this._saveTimer = null; }
                this._persistNow();
            };
        ...
        _persistNow() {
            const budget = Config.replayHistoryBudgetBytes;
            const entries = this._entries.slice();
            const measure = () => { try { return JSON.stringify(entries).length; } catch { return 0; } };
            while (entries.length && measure() > budget) entries.shift();
    ...
    const PageInspector = {
        ...
        searchCssClasses(query, limit) {
            limit = limit || 200; 
            const OCCURRENCES_PER_CLASS = 8; 
            const q = (query || '').toLowerCase().trim();
            const classMap = new Map();
            let skippedSheets = 0;

            const walkRules = (rules, sheetLabel) => {
                if (!rules) return;
                for (let i = 0; i < rules.length; i++) {
                    const rule = rules[i];
                    if (rule.selectorText && rule.style) {
                        const classNames = rule.selectorText.match(/\.[a-zA-Z_][a-zA-Z0-9_-]*/g) || [];
                        const uniqueInRule = new Set(classNames.map((c) => c.slice(1)));
                        uniqueInRule.forEach((cls) => {
                            if (!q || cls.toLowerCase().includes(q)) {
                                if (!classMap.has(cls)) {
                                    if (classMap.size >= limit) return;
                                    classMap.set(cls, []);
                                }
                                const occurrences = classMap.get(cls);
                                if (occurrences.length < OCCURRENCES_PER_CLASS) {
                                    let properties = '';
                                    try { properties = rule.style.cssText; } catch {  }
                                    if (properties.length > 200) properties = properties.slice(0, 200) + '...';
                                    occurrences.push({ selector: rule.selectorText, properties, source: sheetLabel });
                                }
                            }
                        });
                    } else if (rule.cssRules) {
                        walkRules(rule.cssRules, sheetLabel); 
                    }
                }
            };
            ...
            for (let i = 0; i < document.styleSheets.length; i++) {
                ...
                try {
                    walkRules(sheet.cssRules, sheet.href ? sheet.href.split('/').pop() : '(inline)');
                } catch {
                    skippedSheets++;
                }
            }

            const results = Array.from(classMap.entries()).map(([className, occurrences]) => ({ className, occurrences }));
            return { results, skippedSheets, totalSheets: document.styleSheets.length, occurrencesPerClassCap: OCCURRENCES_PER_CLASS };
        },
    ...
    const Recon = {
        ...
        _misconfigCache: null,
        _pageHeaders: null,
        _pageHeadersError: null,
        _pageHeadersFetching: false,

        SIGNATURES: [
            { name: 'WordPress', category: 'CMS', test: (c) => c.scriptSrcs.some((s) => /\/wp-content\/|\/wp-includes\//.test(s)) || c.metaGenerator.toLowerCase().includes('wordpress') || c.globals.includes('wp'),
              evidence: (c) => [c.metaGenerator.toLowerCase().includes('wordpress') && 'meta generator tag', c.scriptSrcs.some((s) => /\/wp-content\/|\/wp-includes\//.test(s)) && 'wp-content/wp-includes script paths'].filter(Boolean) },
            { name: 'Shopify', category: 'CMS', test: (c) => c.globals.includes('Shopify') || c.scriptSrcs.some((s) => /cdn\.shopify\.com/.test(s)),
              evidence: (c) => [c.globals.includes('Shopify') && 'window.Shopify global', c.scriptSrcs.some((s) => /cdn\.shopify\.com/.test(s)) && 'cdn.shopify.com script source'].filter(Boolean) },
            { name: 'Drupal', category: 'CMS', test: (c) => c.globals.includes('Drupal') || c.metaGenerator.toLowerCase().includes('drupal'),
              evidence: (c) => [c.globals.includes('Drupal') && 'window.Drupal global', c.metaGenerator.toLowerCase().includes('drupal') && 'meta generator tag'].filter(Boolean) },
            { name: 'Wix', category: 'CMS', test: (c) => c.scriptSrcs.some((s) => /static\.wixstatic\.com|static\.parastorage\.com/.test(s)),
              evidence: () => ['wixstatic/parastorage script source'] },
            { name: 'React', category: 'JS Framework', test: (c) => c.globals.includes('React') || c.globals.includes('__REACT_DEVTOOLS_GLOBAL_HOOK__') || c.html.includes('data-reactroot') || c.html.includes('__NEXT_DATA__'),
              evidence: (c) => [c.globals.includes('React') && 'window.React global', c.globals.includes('__REACT_DEVTOOLS_GLOBAL_HOOK__') && 'React DevTools hook', c.html.includes('data-reactroot') && 'data-reactroot attribute'].filter(Boolean) },
            { name: 'Next.js', category: 'JS Framework', test: (c) => c.html.includes('__NEXT_DATA__') || c.scriptSrcs.some((s) => /\/_next\//.test(s)),
              evidence: (c) => [c.html.includes('__NEXT_DATA__') && '__NEXT_DATA__ payload', c.scriptSrcs.some((s) => /\/_next\//.test(s)) && '/_next/ script paths'].filter(Boolean) },
            { name: 'Vue.js', category: 'JS Framework', test: (c) => c.globals.includes('Vue') || c.html.includes('data-v-') || c.globals.includes('__VUE__'),
              evidence: (c) => [c.globals.includes('Vue') && 'window.Vue global', c.html.includes('data-v-') && 'data-v- scoped-style attributes'].filter(Boolean) },
            { name: 'Nuxt.js', category: 'JS Framework', test: (c) => c.globals.includes('__NUXT__'),
              evidence: () => ['window.__NUXT__ global'] },
            { name: 'Angular', category: 'JS Framework', test: (c) => c.globals.includes('ng') || c.globals.includes('getAllAngularRootElements') || c.html.includes('ng-version'),
              evidence: (c) => [c.html.includes('ng-version') && 'ng-version attribute', c.globals.includes('getAllAngularRootElements') && 'Angular root-element debug hook'].filter(Boolean) },
            { name: 'jQuery', category: 'JS Library', test: (c) => c.globals.includes('jQuery') || c.globals.includes('$'),
              evidence: (c) => [c.globals.includes('jQuery') && 'window.jQuery global'].filter(Boolean) },
            { name: 'Svelte', category: 'JS Framework', test: (c) => c.html.includes('svelte-') , evidence: () => ['svelte- scoped-style class names'] },
            { name: 'Ember.js', category: 'JS Framework', test: (c) => c.globals.includes('Ember'), evidence: () => ['window.Ember global'] },
            { name: 'Alpine.js', category: 'JS Library', test: (c) => c.globals.includes('Alpine') || c.html.includes('x-data'), evidence: (c) => [c.globals.includes('Alpine') && 'window.Alpine global', c.html.includes('x-data') && 'x-data attribute'].filter(Boolean) },
            { name: 'htmx', category: 'JS Library', test: (c) => c.globals.includes('htmx') || c.html.includes('hx-get') || c.html.includes('hx-post'), evidence: (c) => [c.globals.includes('htmx') && 'window.htmx global'].filter(Boolean) },
            { name: 'Lodash', category: 'JS Library', test: (c) => c.globals.includes('_') && c.scriptSrcs.some((s) => /lodash/.test(s)), evidence: () => ['lodash script source + window._ global'] },
            { name: 'Redux', category: 'JS Library', test: (c) => c.globals.includes('__REDUX_DEVTOOLS_EXTENSION__'), evidence: () => ['Redux DevTools hook'] },
            { name: 'Bootstrap', category: 'CSS Framework', test: (c) => c.globals.includes('bootstrap') || c.scriptSrcs.some((s) => /bootstrap(\.min)?\.js/.test(s)), evidence: (c) => [c.scriptSrcs.some((s) => /bootstrap/.test(s)) && 'bootstrap script source'].filter(Boolean) },
            { name: 'Google Analytics / GA4', category: 'Analytics', test: (c) => c.globals.includes('gtag') || c.globals.includes('ga') || c.globals.includes('dataLayer'), evidence: (c) => [c.globals.includes('gtag') && 'window.gtag global', c.globals.includes('dataLayer') && 'window.dataLayer global'].filter(Boolean) },
            { name: 'Google Tag Manager', category: 'Analytics', test: (c) => c.scriptSrcs.some((s) => /googletagmanager\.com/.test(s)), evidence: () => ['googletagmanager.com script source'] },
            { name: 'Meta/Facebook Pixel', category: 'Analytics', test: (c) => c.globals.includes('fbq'), evidence: () => ['window.fbq global'] },
            { name: 'Sentry', category: 'Monitoring', test: (c) => c.globals.includes('Sentry') || c.globals.includes('Raven'), evidence: (c) => [c.globals.includes('Sentry') && 'window.Sentry global'].filter(Boolean) },
            { name: 'Segment', category: 'Analytics', test: (c) => c.globals.includes('analytics') && c.scriptSrcs.some((s) => /cdn\.segment\.com/.test(s)), evidence: () => ['segment.com script source'] },
            { name: 'Hotjar', category: 'Analytics', test: (c) => c.globals.includes('hj') || c.scriptSrcs.some((s) => /static\.hotjar\.com/.test(s)), evidence: () => ['hotjar signal'] },
            { name: 'reCAPTCHA', category: 'Security/Captcha', test: (c) => c.globals.includes('grecaptcha') || c.scriptSrcs.some((s) => /recaptcha/.test(s)), evidence: () => ['recaptcha script or global'] },
            { name: 'hCaptcha', category: 'Security/Captcha', test: (c) => c.globals.includes('hcaptcha') || c.scriptSrcs.some((s) => /hcaptcha\.com/.test(s)), evidence: () => ['hcaptcha script or global'] },
            { name: 'Cloudflare Turnstile', category: 'Security/Captcha', test: (c) => c.globals.includes('turnstile') || c.scriptSrcs.some((s) => /challenges\.cloudflare\.com/.test(s)), evidence: () => ['turnstile script or global'] },
            { name: 'Stripe', category: 'Payments', test: (c) => c.globals.includes('Stripe') || c.scriptSrcs.some((s) => /js\.stripe\.com/.test(s)), evidence: () => ['stripe script or global'] },
            { name: 'Cloudflare', category: 'Infra/CDN', test: (c) => /cloudflare/i.test(c.headerBlob), evidence: () => ['server/cf-ray header seen in traffic'] },
            { name: 'nginx', category: 'Infra/CDN', test: (c) => /nginx/i.test(c.headerBlob), evidence: () => ['Server header seen in traffic'] },
            { name: 'Apache', category: 'Infra/CDN', test: (c) => /apache/i.test(c.headerBlob), evidence: () => ['Server header seen in traffic'] },
            { name: 'Vercel', category: 'Infra/CDN', test: (c) => /vercel/i.test(c.headerBlob), evidence: () => ['x-vercel-* header seen in traffic'] },
        ...
        _buildContext() {
            const scriptSrcs = [];
            try { document.querySelectorAll('script[src]').forEach((s) => scriptSrcs.push(s.src)); } catch {  }
            let metaGenerator = '';
            try { metaGenerator = (document.querySelector('meta[name="generator"]') || {}).content || ''; } catch {  }
            let html = '';
            try { html = document.documentElement.outerHTML.slice(0, 400000); } catch {  }
            const globals = [];
            const realWindow = (typeof getRealWindow === 'function') ? getRealWindow() : window;
        ...
        _GLOBAL_LOCATE: {
            'React': 'React', 'Vue.js': 'Vue', 'Nuxt.js': '__NUXT__', 'Angular': 'ng', 'jQuery': 'jQuery',
        ...
        _SCRIPT_LOCATE: {
            ...
            'Wix': /static\.wixstatic\.com|static\.parastorage\.com/, 'Next.js': /\/_next\//,
        ...
        _locateForHit(ctx, name) {
            ...
            if (scriptRe) {
                ...
                if (match) return { kind: 'script', src: match };
            }
        ...
        fingerprint(force) {
            if (this._fingerprintCache && !force) return this._fingerprintCache;
            const ctx = this._buildContext();
            const hits = [];
            this.SIGNATURES.forEach((sig) => {
                let matched = false;
                try { matched = !!sig.test(ctx); } catch {  }
                if (!matched) return;
                let evidence = [];
            ...
            hits.sort((a, b) => {
                ...
                return order[a.confidence] - order[b.confidence] || a.name.localeCompare(b.name);
            });
            this._fingerprintCache = hits;
            return hits;
        },

        async fetchPageHeaders() {
            this._pageHeadersFetching = true;
            this._pageHeadersError = null;
            try {
                const res = await fetch(location.href, { method: 'GET', cache: 'no-store', credentials: 'same-origin' });
                const headers = {};
                try { res.headers.forEach((v, k) => { headers[k] = v; }); } catch {  }
                this._pageHeaders = headers;
                this._pageHeadersFetchedAt = Date.now();
                if (!Object.keys(headers).length) {
                    this._pageHeadersError = 'Request succeeded but no headers were readable (possibly an opaque or cross-origin redirect response).';
                }
                return headers;
            } catch (e) {
                this._pageHeadersError = (e && e.message) || String(e);
                return null;
            } finally {
                this._pageHeadersFetching = false;
                this._misconfigCache = null;
            }
        },

        SECURITY_HEADER_CHECKS: [
            { header: 'content-security-policy', label: 'Content-Security-Policy', severity: 'high', note: 'No CSP - reduces the browser\u2019s ability to contain injected script/content.' },
            { header: 'x-frame-options', label: 'X-Frame-Options', severity: 'medium', note: 'No clickjacking protection via X-Frame-Options (a CSP frame-ancestors directive can also cover this - checked separately).' },
            { header: 'strict-transport-security', label: 'Strict-Transport-Security', severity: 'medium', note: 'No HSTS - downgrade/strip attacks on future connections aren\u2019t mitigated.', httpsOnly: true },
            { header: 'x-content-type-options', label: 'X-Content-Type-Options', severity: 'low', note: 'No nosniff - browsers may MIME-sniff responses.' },
            { header: 'referrer-policy', label: 'Referrer-Policy', severity: 'low', note: 'No explicit Referrer-Policy - default browser behavior applies.' },
            { header: 'permissions-policy', label: 'Permissions-Policy', severity: 'low', note: 'No Permissions-Policy - browser feature access isn\u2019t explicitly scoped.' }
        ],

        ERROR_BODY_PATTERNS: [
            { re: /Traceback \(most recent call last\)/, label: 'Python traceback' },
            { re: /at\s+[\w.$]+\s+\([^)]*:\d+:\d+\)/, label: 'Node.js/V8 stack trace' },
            { re: /Fatal error:.*on line \d+/i, label: 'PHP fatal error' },
            { re: /Exception in thread/, label: 'Java stack trace' },
            { re: /System\.\w+Exception/, label: '.NET exception' },
            { re: /DEBUG\s*=\s*True/, label: 'Django DEBUG=True indicator' },
            { re: /Whoops\\\\Exception|Laravel\\\\/, label: 'Laravel/Whoops error page' },
            { re: /SQLSTATE\[/, label: 'Raw SQL error (SQLSTATE)' }
        ],

        misconfigAudit(force) {
            if (this._misconfigCache && !force) return this._misconfigCache;
            const findings = [];
            const isHttps = location.protocol === 'https:';

            const headers = this._pageHeaders || (ObservedTraffic.all().find((e) => {
                try { return new URL(e.url, location.href).pathname === location.pathname && new URL(e.url, location.href).host === location.host; } catch { return false; }
            }) || {}).responseHeaders || null;

            if (headers) {
                this.SECURITY_HEADER_CHECKS.forEach((chk) => {
                    if (chk.httpsOnly && !isHttps) return;
                    const val = ObservedTraffic.getHeader(headers, chk.header);
                    if (!val) findings.push({ kind: 'header', severity: chk.severity, label: `Missing ${chk.label}`, detail: chk.note, locate: { kind: 'headers' } });
                });
                const csp = ObservedTraffic.getHeader(headers, 'content-security-policy');
                if (csp && !/frame-ancestors/i.test(csp)) {
                    findings.push({ kind: 'header', severity: 'low', label: 'CSP present but no frame-ancestors directive', detail: 'CSP exists but doesn\u2019t restrict framing; combine with X-Frame-Options if that\u2019s also missing.', locate: { kind: 'headers' } });
                }
                const server = ObservedTraffic.getHeader(headers, 'server');
                const poweredBy = ObservedTraffic.getHeader(headers, 'x-powered-by');
                if (poweredBy) findings.push({ kind: 'disclosure', severity: 'info', label: 'X-Powered-By disclosed', detail: `Server-side stack disclosed via header: ${poweredBy}`, locate: { kind: 'headers' } });
                if (server && /\d/.test(server)) findings.push({ kind: 'disclosure', severity: 'info', label: 'Server header includes a version number', detail: `Server: ${server}`, locate: { kind: 'headers' } });
            } else {
                findings.push({ kind: 'meta', severity: 'info', label: 'No response headers available yet', detail: 'Use "Check this page\u2019s headers" below, or browse so traffic history captures a same-origin response.', locate: null });
            }

            let cookies = [];
            try { cookies = PageInspector.getCookies(); } catch {  }
            cookies.forEach((c) => {
                findings.push({ kind: 'cookie', severity: 'low', label: `Cookie "${c.key}" lacks HttpOnly`, detail: 'Visible via document.cookie, so it\u2019s readable by any script on the page (including injected/XSS script).', locate: { kind: 'cookie', key: c.key } });
            });

            const seen = new Set();
            ObservedTraffic.all().forEach((entry) => {
                if (!entry.status || entry.status < 400) return;
                const bodyText = entry.rawText || (entry.json ? JSON.stringify(entry.json) : '') || '';
                if (!bodyText) return;
                this.ERROR_BODY_PATTERNS.forEach((p) => {
                    if (p.re.test(bodyText) && !seen.has(p.label + entry.url)) {
                        seen.add(p.label + entry.url);
                        findings.push({ kind: 'error-disclosure', severity: 'medium', label: `Verbose error body: ${p.label}`, detail: `${entry.method} ${Helpers._shortenUrl(entry.url)} (status ${entry.status})`, locate: { kind: 'traffic', url: entry.url } });
                    }
                });
            });
    ...
    const Findings = {
        ...
        _scoreSweepReasonTypes(reasonTypes) {
            const unique = Array.from(new Set(reasonTypes || []));
            let score = 0;
            unique.forEach((t) => { score += this.SWEEP_REASON_WEIGHT[t] || 10; });
            if (unique.length >= 2) score += 10;
            if (unique.length >= 3) score += 10;
        ...
        gather(force) {
            ...
            Recon.misconfigAudit(force).forEach((m) => {
                const score = this.MISCONFIG_SEVERITY_SCORE[m.severity] ?? 20;
                findings.push({
                    source: 'misconfig',
                    kind: m.kind,
                    label: m.label,
            ...
            Recon.fingerprint(force).forEach((f) => {
                const score = this.FINGERPRINT_CONFIDENCE_SCORE[f.confidence] ?? 20;
                findings.push({
                    source: 'fingerprint',
                    kind: 'tech',
            ...
            ReplayHistory.all().forEach((e) => {
                if (!e.sweep || !e.sweep.anomaly) return;
                const score = this._scoreSweepReasonTypes(e.sweep.anomalyReasonTypes);
                findings.push({
                    source: 'sweep',
                    kind: 'injection-anomaly',
                    label: `Anomalous response to swept payload: ${e.sweep.value}`,
                    detail: (e.sweep.anomalyReasons || []).join('; '),
                    score,
                    confidence: this._scoreToBucket(score),
    ...
    const IndexedDBInspector = {
        ...
        async snapshotAllRows(maxRecordsPerStore, maxValueChars) {
            const rows = [];
            if (!this.supported()) return rows;
            let dbs;
            try { dbs = await this.listDatabases(); } catch { return rows; }
            for (const d of dbs) {
                let stores;
                try { stores = await this.listObjectStores(d.name); } catch { continue; }
                for (const s of stores) {
                    let data;
                    try { data = await this.getRecords(d.name, s.name, maxRecordsPerStore); } catch { continue; }
                    data.values.forEach((v, i) => {
                        let valueText;
                        try { valueText = JSON.stringify(v); } catch { valueText = String(v); }
                        if (valueText === undefined) valueText = String(v);
                        if (valueText.length > maxValueChars) valueText = valueText.slice(0, maxValueChars) + '...';
                        let keyText;
                        try { keyText = JSON.stringify(data.keys[i]); } catch { keyText = String(data.keys[i]); }
                        rows.push({ source: `indexeddb:${d.name}:${s.name}`, key: keyText, value: valueText });
                    });
                }
            }
            return rows;
    ...
    const ActiveDiscovery = {
        ...
        DEFAULT_WORDLIST: [
            '/robots.txt', '/sitemap.xml', '/.well-known/security.txt', '/.well-known/change-password',
            '/.git/HEAD', '/.git/config', '/.svn/entries', '/.hg/hgrc',
            '/.env', '/.env.local', '/.env.production', '/.env.example',
            '/config.json', '/config.php', '/configuration.php', '/settings.json',
            '/wp-login.php', '/wp-admin/', '/wp-json/', '/wp-config.php.bak',
            '/administrator/', '/admin/', '/admin.php', '/admin/login',
            '/phpinfo.php', '/info.php', '/server-status', '/server-info',
            '/.htaccess', '/.htpasswd', '/web.config',
            '/api/', '/api/v1/', '/api/v2/', '/graphql', '/swagger.json', '/swagger-ui.html', '/openapi.json',
            '/backup.zip', '/backup.sql', '/backup.tar.gz', '/db.sql', '/database.sql', '/dump.sql',
            '/.DS_Store', '/composer.json', '/package.json', '/package-lock.json', '/yarn.lock',
            '/webpack.config.js', '/.npmrc', '/.aws/credentials', '/id_rsa', '/id_rsa.pub',
            '/debug', '/test', '/staging', '/dev', '/status', '/health', '/healthz', '/metrics',
            '/actuator/health', '/version', '/version.json', '/CHANGELOG.md', '/README.md', '/LICENSE',
        ...
        loadSavedWordlists() {
            return Storage.get(this.WORDLIST_STORAGE_KEY, {}) || {};
        },

        saveWordlist(name, text) {
            const all = this.loadSavedWordlists();
            all[name] = text;
            Storage.set(this.WORDLIST_STORAGE_KEY, all);
            return all;
        },
        ...
        runPathDiscovery(baseUrl, paths, delayMs, mode, callbacks) {
            callbacks = callbacks || {};
            let stopped = false;
            let idx = 0;
            ...
            const runNext = () => {
                if (stopped) { callbacks.onDone && callbacks.onDone(true); return; }
                if (idx >= paths.length) { callbacks.onDone && callbacks.onDone(false); return; }
        ...
        collectLinksFromHtml(html, baseUrl) {
            const links = new Set();
            try {
        ...
        collectLinksFromCurrentPage() {
            return this.collectLinksFromHtml(document.documentElement.outerHTML, location.href);
        },

        extractScriptsFromHtml(html, baseUrl) {
            const results = [];
            try {
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ...
                for (let i = 0; i < scripts.length; i++) {
                    ...
                    if (srcAttr) {
                        let absSrc = srcAttr;
                        try { absSrc = new URL(srcAttr, baseUrl).href; } catch {  }
                        results.push({ index: i, external: true, src: absSrc, length: null, preview: null, hints: [], full: null, fetchError: null });
                        continue;
                    }
                    const text = s.textContent || '';
                    if (!text.trim()) continue;
                    ...
                    const assignRe = /(?:var|let|const|window\.)\s*([A-Za-z_$][\w$]*)\s*=\s*(\{|\[)/g;
                    let match;
                    while ((match = assignRe.exec(text)) && hints.length < 8) hints.push(match[1]);
                    results.push({ index: i, external: false, src: null, length: text.length, preview: text.slice(0, 300), hints, full: text, fetchError: null });
                }
            } catch {  }
            return results;
        ...
        runCrawl(urls, delayMs, mode, callbacks) {
            callbacks = callbacks || {};
            let stopped = false;
            let idx = 0;
            const wait = Math.max(this.MIN_DELAY_MS, Number(delayMs) || 0);
            ...
            const runNext = () => {
                if (stopped) { callbacks.onDone && callbacks.onDone(true); return; }
                if (idx >= urls.length) { callbacks.onDone && callbacks.onDone(false); return; }
                const url = urls[idx];
                const stepIndex = idx;
                idx++;
                ...
                const doSend = async () => {
                    if (stopped) { callbacks.onDone && callbacks.onDone(true); return; }
                    const result = await RequestReplay.sendWithMode({ method: 'GET', url, headersText: '', body: '' }, mode);
                    this._recordEntry(url, 'GET', result);
                    let newLinks = [];
                    if (result.ok && !result.json && result.rawText) newLinks = this.collectLinksFromHtml(result.rawText, url);
                    callbacks.onStep && callbacks.onStep(stepIndex, url, result, newLinks);
                    runNext();
                };
    ...
    const Helpers = {
        ...
        _primaryBtnStyleInline() {
            return 'padding:8px 12px;background:linear-gradient(135deg, #3f8296, #1e4550);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600;white-space:nowrap;flex:0 0 auto;';
        },
    ...
    const PageInspectorUI = {
        ...
        _highlightGlobalKey: null,

        _discoveryBaseUrl: '',
        _discoveryWordlistText: '',
        _discoveryDelay: 400,
        ...
        _sectionGroups: [
            {
                ...
                sections: [
                    { value: 'traffic', label: 'Traffic history (all requests)' },
                    { value: 'endpoints', label: 'Observed endpoints (auto-catalog)' },
                    { value: 'network', label: 'Network / resource timing' },
                    { value: 'waterfall', label: 'Network waterfall' },
                    { value: 'pageload', label: 'Page load timing' },
                    { value: 'pageloadcompare', label: 'Compare traffic across page loads' },
                    { value: 'hosts', label: 'Hosts summary' },
                    { value: 'duplicates', label: 'Duplicate requests' },
                    { value: 'websocket', label: 'WebSocket activity' }
                ]
            },
            {
                id: 'page', label: 'Page & DOM',
                sections: [
                    { value: 'dom', label: 'DOM (full page HTML)' },
                    { value: 'element', label: 'Element inspector' },
                    { value: 'eventdebug', label: 'Event Debugger' },
                    { value: 'domwatch', label: 'DOM Mutation Watcher' },
                    { value: 'classsearch', label: 'CSS Class Search' },
                    { value: 'scripts', label: 'Script sources' }
                ]
            },
            {
                id: 'storage', label: 'Storage & Data',
                sections: [
                    { value: 'storage', label: 'localStorage / sessionStorage' },
                    { value: 'cookies', label: 'Cookies' },
                    { value: 'storagevault', label: 'Storage/Cookie Vault' },
                    { value: 'storagewatch', label: 'Storage/Cookie Watcher' },
                    { value: 'indexeddb', label: 'IndexedDB' },
                    { value: 'globals', label: 'window globals' }
                ]
            },
            {
            ...
            {
                id: 'testing', label: 'Testing & Automation',
                sections: [
                    { value: 'console', label: 'JS Console' },
                    { value: 'recorder', label: 'Investigation Recorder' },
                    { value: 'snapshots', label: 'Snapshots (compare)' },
                    { value: 'tracer', label: 'Value Tracer' },
                    { value: 'tokens', label: 'Token Inspector' }
                ]
            },
            {
                id: 'export', label: 'Export / AI Briefing',
                sections: [
                    { value: 'export', label: 'Export / AI Briefing' }
                ]
            }
        ],

        _categoryForSection(value) {
            const group = this._sectionGroups.find((g) => g.sections.some((s) => s.value === value));
            return group ? group.id : this._sectionGroups[0].id;
        },

        _sectionLabel(value, staticLabel) {
            if (value === 'websocket') return `WebSocket activity${Config.wsCaptureEnabled ? '' : ' (off)'}`;
            return staticLabel;
        },
        ...
        render() {
            ...
            if (!this._activeCategory) this._activeCategory = this._categoryForSection(this._section);
            const activeGroup = this._sectionGroups.find((g) => g.id === this._activeCategory) || this._sectionGroups[0];

            body.innerHTML = `
                <div style="${Helpers._cardStyle(t)}">
                    <select id="ttd-pi-category" style="width:100%;padding:7px;margin-bottom:8px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:12px;">
                        ${this._sectionGroups.map((g) => `<option value="${g.id}" ${g.id === activeGroup.id ? 'selected' : ''}>${g.label}</option>`).join('')}
                    </select>
                    ${activeGroup.sections.length > 1 ? `
                    <div id="ttd-pi-tabs" style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px;">
                        ${activeGroup.sections.map((s) => `<button data-tab="${s.value}" style="${Helpers._pillStyle(t, this._section === s.value)}font-size:10px;padding:5px 8px;">${this._sectionLabel(s.value, s.label)}</button>`).join('')}
                    </div>` : ''}
                    <div id="ttd-pi-content"></div>
                </div>
            `;

            document.getElementById('ttd-pi-category').onchange = (e) => {
                const group = this._sectionGroups.find((g) => g.id === e.target.value);
                this._activeCategory = e.target.value;
                this._goToSection(group.sections[0].value);
            };
            const tabsEl = document.getElementById('ttd-pi-tabs');
            if (tabsEl) {
                tabsEl.querySelectorAll('[data-tab]').forEach((btn) => {
                    btn.onclick = () => this._goToSection(btn.getAttribute('data-tab'));
                });
            }
            this._renderSection();
        },
        ...
        _renderSection() {
            ...
            if (this._section === 'storagevault') return this._renderStorageVault();
            if (this._section === 'indexeddb') return this._renderIndexedDB();
            if (this._section === 'classsearch') return this._renderCssClassSearch();
        ...
        _renderDom() {
            ...
            area.innerHTML = `
                ${this._domSnapshotSource ? `<div style="font-size:10px;color:${t.statusWarn};margin-bottom:6px;word-break:break-all;">Viewing a crawled page, not the live one: ${Helpers._escape(this._domSnapshotSource.url)} (fetched ${Helpers._timeAgo(this._domSnapshotSource.fetchedAt)}) - <span id="ttd-pi-dom-back-live" style="text-decoration:underline;cursor:pointer;">back to live page</span></div>` : ''}
            ...
            const backLive = document.getElementById('ttd-pi-dom-back-live');
            if (backLive) backLive.onclick = () => { this._domSnapshot = PageInspector.getDomHtml(); this._domSnapshotSource = null; this._renderDom(); };
            document.getElementById('ttd-pi-dom-refresh').onclick = () => { this._domSnapshot = PageInspector.getDomHtml(); this._domSnapshotSource = null; this._renderDom(); };
        ...
        _renderSnapshots() {
            ...
            area.innerHTML = `
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:8px;">Captures localStorage, sessionStorage, cookies, and window globals as they stand right now, so you can diff two points in time against each other later - before/after a reload, before/after an action. Persists across reloads, capped at ${SnapshotManager.MAX_SNAPSHOTS} - for anything you want to keep longer or share elsewhere, use Export/AI Briefing's DOM snapshot category instead.</div>
        ...
        _renderSnapshotList() {
            ...
            listEl.innerHTML = snaps.map((s) => `
                <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 8px;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:4px;font-size:11px;">
                    ...
                    <div style="display:flex;gap:4px;flex:0 0 auto;margin-left:6px;">
                        <button data-restore-snapshot="${s.id}" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;color:${t.statusWarn};">Restore</button>
                        <button data-remove-snapshot="${s.id}" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Remove</button>
                    </div>
                </div>
            `).join('');
            ...
            listEl.querySelectorAll('[data-restore-snapshot]').forEach((btn) => {
                btn.onclick = () => {
                    const id = parseInt(btn.getAttribute('data-restore-snapshot'), 10);
                    const snap = SnapshotManager.get(id);
                    if (!snap) return;
                    if (!confirm(`Restore "${snap.label}"? This writes its localStorage/sessionStorage/cookies back onto the live page right now. Keys that exist now but weren't in this snapshot are left alone - this isn't a full wipe-and-replace. Globals and IndexedDB aren't part of a snapshot and won't be touched.`)) return;
                    const result = SnapshotManager.restore(id);
                    btn.textContent = result.ok ? `Restored ${result.count}` : `${result.errors.length} failed`;
        ...
        _renderCssClassSearch() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-content');
            const canBackToElement = this._pickedElement && this._pickedElement.isConnected;
            area.innerHTML = `
                ${canBackToElement ? `<button id="ttd-pi-classsearch-back" style="${Helpers._secondaryBtnStyle(t)}margin-bottom:8px;">&larr; Back to element</button>` : ''}
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:8px;">Searches every accessible stylesheet's class selectors for a substring match - built for CSS-Modules-style names like "stockOwned___VshhT", where the readable part is guessable but the hash suffix isn't. That hash is typically build-derived and can change on a new deploy, so treat a found class as good for testing now, not something to hardcode into a script meant to survive Torn's next release.</div>
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <input id="ttd-pi-classsearch-input" type="text" placeholder="e.g. owned, equipped, active, error" value="${Helpers._escape(this._cssClassSearchQuery || '')}" style="flex:1;min-width:0;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:12px;">
                    <button id="ttd-pi-classsearch-go" style="${Helpers._primaryBtnStyleInline()}">Search</button>
                </div>
                <div id="ttd-pi-classsearch-results"></div>
            `;

            if (canBackToElement) document.getElementById('ttd-pi-classsearch-back').onclick = () => this._goToSection('element');

            const input = document.getElementById('ttd-pi-classsearch-input');
            const run = () => {
                this._cssClassSearchQuery = input.value;
                this._cssClassSearchResult = PageInspector.searchCssClasses(input.value, 200);
                this._renderCssClassSearchResults();
            };
            document.getElementById('ttd-pi-classsearch-go').onclick = run;
            input.onkeydown = (e) => { if (e.key === 'Enter') run(); };

            if (this._cssClassSearchResult) this._renderCssClassSearchResults();
        },

        _renderCssClassSearchResults() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-classsearch-results');
            if (!area) return;
            const data = this._cssClassSearchResult;
            if (!data) { area.innerHTML = ''; return; }

            const skippedNote = data.skippedSheets
                ? `<div style="font-size:10px;color:${t.statusWarn};margin-bottom:6px;">${data.skippedSheets} of ${data.totalSheets} stylesheets couldn't be read (cross-origin, no CORS headers) - a class defined only in one of those won't show up here.</div>`
                : '';

            if (!data.results.length) {
                area.innerHTML = `${skippedNote}<div style="font-size:11px;color:${t.cardDesc};">No matching class selectors found.</div>`;
                return;
            }

            const canApply = this._pickedElement && this._pickedElement.isConnected;
            const totalOccurrences = data.results.reduce((sum, r) => sum + r.occurrences.length, 0);

            area.innerHTML = `
                ${skippedNote}
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:6px;">${data.results.length}${data.results.length === 200 ? '+' : ''} distinct class${data.results.length === 1 ? '' : 'es'} (${totalOccurrences} rule match${totalOccurrences === 1 ? '' : 'es'})${canApply ? '' : ' - pick an element first to enable "Apply to picked element"'}</div>
                <div id="ttd-pi-classsearch-list" style="max-height:320px;overflow-y:auto;"></div>
            `;

            const listEl = document.getElementById('ttd-pi-classsearch-list');
            data.results.forEach((r) => {
                const row = document.createElement('div');
                row.style.cssText = `padding:6px 8px;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:4px;font-size:11px;word-break:break-all;`;
                const occHtml = r.occurrences.map((occ) => `
                    <div style="font-size:9px;color:${t.cardDesc};margin-bottom:4px;padding-left:6px;border-left:2px solid ${t.rowBorder};">
                        <div>${Helpers._escape(occ.selector)} - ${Helpers._escape(occ.source)}</div>
                        ${occ.properties ? `<div style="font-family:monospace;">${Helpers._escape(occ.properties)}</div>` : ''}
                    </div>
                `).join('');
                const moreNote = r.occurrences.length >= data.occurrencesPerClassCap
                    ? `<div style="font-size:9px;color:${t.cardDesc};margin-bottom:4px;">(showing first ${data.occurrencesPerClassCap} rules for this class - it may appear in more)</div>`
                    : '';
                row.innerHTML = `
                    <div style="font-weight:700;margin-bottom:4px;">${Helpers._escape(r.className)} <span style="font-weight:400;color:${t.cardDesc};">- ${r.occurrences.length} rule${r.occurrences.length === 1 ? '' : 's'}</span></div>
                    ${occHtml}
                    ${moreNote}
                    <div style="display:flex;gap:6px;">
                        <button data-copy-class style="${Helpers._secondaryBtnStyle(t)}">Copy class name</button>
                        <button data-apply-class style="${Helpers._secondaryBtnStyle(t)}" ${canApply ? '' : 'disabled'}>Add to picked element</button>
                    </div>
                `;
                row.querySelector('[data-copy-class]').onclick = async (e) => {
                    const ok = await copyToClipboard(r.className);
                    e.target.textContent = ok ? 'Copied!' : 'Failed';
                    setTimeout(() => { if (e.target.isConnected) e.target.textContent = 'Copy class name'; }, 1200);
                };
                const applyBtn = row.querySelector('[data-apply-class]');
                if (canApply) {
                    applyBtn.onclick = () => {
                        this._pickedElement.classList.add(r.className);
                        applyBtn.textContent = 'Added!';
                        setTimeout(() => { if (applyBtn.isConnected) applyBtn.textContent = 'Add to picked element'; }, 1200);
                    };
                }
        ...
        _renderElementEditor(pickedEl, t) {
            ...
            return `
                <div style="border:1px solid ${t.rowBorder};border-radius:6px;padding:8px;margin-bottom:8px;">
                    ...
                    <div style="display:flex;gap:6px;margin-bottom:8px;">
                        <button id="ttd-pi-element-save-attrs" style="${Helpers._secondaryBtnStyle(t)}">Save attributes</button>
                        <button id="ttd-pi-element-find-class" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Find a class...</button>
                    </div>
        ...
        _wireElementEditor(pickedEl, t) {
            ...
            const guardConnected = () => {
                if (pickedEl.isConnected) return null;
                return { ok: false, error: 'This element is no longer attached to the page - something (likely a framework re-render) replaced it with a different node. Go back and pick the element again to edit the current one.' };
            };
            ...
            const safely = (fn) => {
                try {
                    ...
                    report({ ok: false, error: `Unexpected error: ${(e && e.message) || String(e)}` });
                    return undefined;
                }
            };

            const verifyStuck = (getMismatches, label, onDone) => {
                setTimeout(() => {
                    const s = statusEl();
                    const mismatches = getMismatches();
                    if (s && mismatches && mismatches.length) {
                        s.textContent = `Applied, but reverted moments later (${label}: ${mismatches.join(', ')}) - something on the page, often a JS framework re-rendering this element, overwrote it. Direct DOM edits on framework-managed elements often don't stick.`;
                        s.style.color = t.statusBad;
            ...
            document.getElementById('ttd-pi-element-find-class').onclick = () => this._goToSection('classsearch');

            document.getElementById('ttd-pi-element-save-attrs').onclick = () => safely(() => {
                const guard = guardConnected();
                if (guard) { report(guard); return; }
                const text = document.getElementById('ttd-pi-element-edit-attrs').value;
                const result = PageInspector.setElementAttributesFromText(pickedEl, text);
                report(result);
                if (result.ok) {
                    verifyStuck(() => Object.keys(result.wanted).filter((name) => pickedEl.getAttribute(name) !== result.wanted[name]), 'attributes', () => this._renderElement());
                } else {
                    setTimeout(() => this._renderElement(), 1500);
                }
            });
            document.getElementById('ttd-pi-element-save-style').onclick = () => safely(() => {
                const guard = guardConnected();
                if (guard) { report(guard); return; }
                const text = document.getElementById('ttd-pi-element-edit-style').value;
                const result = PageInspector.setElementInlineStyle(pickedEl, text);
                report(result);
                if (result.ok) {
                    verifyStuck(() => ((pickedEl.getAttribute('style') || '') !== text ? ['style'] : []), 'style', () => this._renderElement());
                } else {
                    setTimeout(() => this._renderElement(), 1500);
                }
            });
            document.getElementById('ttd-pi-element-save-text').onclick = () => safely(() => {
                const guard = guardConnected();
                if (guard) { report(guard); return; }
                const text = document.getElementById('ttd-pi-element-edit-text').value;
                const result = PageInspector.setElementText(pickedEl, text);
                report(result);
                if (result.ok) {
                    verifyStuck(() => ((pickedEl.textContent || '') !== text ? ['text'] : []), 'text content', () => this._renderElement());
                } else {
                    setTimeout(() => this._renderElement(), 1500);
                }
            });
            document.getElementById('ttd-pi-element-remove').onclick = () => safely(() => {
        ...
        _renderTokenVault() {
            ...
            area.innerHTML = `
                <div style="font-size:10px;color:${t.statusBad};margin-bottom:6px;">Persists across reloads. This is real credential material - clear it before sharing anything derived from this browser's storage/backups.</div>
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                    <span style="font-size:10px;color:${t.cardDesc};">${entries.length} of up to ${TokenVault.MAX} saved</span>
                    <div style="display:flex;gap:6px;">
                        <button id="ttd-pi-tokens-vault-add" style="${Helpers._secondaryBtnStyle(t)}">+ Add manually</button>
                        ${entries.length ? `<button id="ttd-pi-tokens-vault-clear" style="${Helpers._secondaryBtnStyle(t)}">Clear vault</button>` : ''}
                    </div>
                </div>
                <div id="ttd-pi-tokens-vault-list" style="max-height:180px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:8px;${this._addingVaultToken ? 'display:none;' : ''}"></div>
                <div id="ttd-pi-tokens-detail"></div>
            `;

            document.getElementById('ttd-pi-tokens-vault-add').onclick = () => {
                this._addingVaultToken = true;
                this._selectedToken = null;
                this._renderTokenVault();
            };
            ...
            if (this._addingVaultToken) {
                this._renderTokenVaultAddForm();
                return;
            }

            const listEl = document.getElementById('ttd-pi-tokens-vault-list');
            if (!entries.length) {
                listEl.innerHTML = `<div style="padding:8px;font-size:11px;color:${t.cardDesc};">Nothing saved yet - open a scanned token and tap "Save to vault" to keep it around even after it rotates or falls out of traffic history, or use "+ Add manually" to paste one in directly.</div>`;
        ...
        _renderTokenVaultAddForm() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-tokens-detail');
            if (!area) return;
            area.innerHTML = `
                <div style="font-size:11px;font-weight:700;margin-bottom:4px;">Add token manually</div>
                <textarea id="ttd-pi-tokens-add-value" placeholder="Token value" style="width:100%;box-sizing:border-box;height:70px;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;font-family:monospace;"></textarea>
                <input id="ttd-pi-tokens-add-label" placeholder="Label (optional)" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                <div style="display:flex;gap:6px;">
                    <button id="ttd-pi-tokens-add-save" style="${Helpers._primaryBtnStyleInline()}">Save</button>
                    <button id="ttd-pi-tokens-add-cancel" style="${Helpers._secondaryBtnStyle(t)}">Cancel</button>
                </div>
                <div id="ttd-pi-tokens-add-status" style="font-size:10px;margin-top:4px;"></div>
            `;
            document.getElementById('ttd-pi-tokens-add-cancel').onclick = () => {
                this._addingVaultToken = false;
                this._renderTokenVault();
            };
            document.getElementById('ttd-pi-tokens-add-save').onclick = () => {
                const value = document.getElementById('ttd-pi-tokens-add-value').value.trim();
                const label = document.getElementById('ttd-pi-tokens-add-label').value;
                const statusEl = document.getElementById('ttd-pi-tokens-add-status');
                if (!value) { statusEl.textContent = 'Value required.'; statusEl.style.color = t.statusBad; return; }
                const entry = TokenVault.addManual(value, label);
                this._addingVaultToken = false;
                this._selectedToken = entry;
                this._renderTokenVault();
            };
        },
        ...
        _renderTokenDetail() {
            ...
            el.innerHTML = `
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;">
                    ...
                    ${this._tokenViewMode === 'vault'
                        ? `<button id="ttd-pi-token-vault-remove" style="${Helpers._secondaryBtnStyle(t)}color:${t.statusBad};">Remove from vault</button><button id="ttd-pi-token-vault-edit" style="${Helpers._secondaryBtnStyle(t)}">Edit value</button>`
            ...
            const vaultEditBtn = document.getElementById('ttd-pi-token-vault-edit');
            if (vaultEditBtn) vaultEditBtn.onclick = () => {
                const newValue = prompt('New token value:', tok.value);
                if (newValue === null) return;
                const trimmed = newValue.trim();
                if (!trimmed) return;
                TokenVault.updateValue(tok.id, trimmed);
                this._selectedToken = TokenVault.all().find((e) => e.id === tok.id) || null;
                this._renderTokenVault();
            };
        ...
        _renderExport() {
            ...
            const tokenVaultCount = TokenVault.all().length;
            ...
            area.innerHTML = `
                ...
                <div style="margin-bottom:8px;">
                    ...
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${replayHistoryCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-replayHistory" ${cats.replayHistory ? 'checked' : ''} ${replayHistoryCount ? '' : 'disabled'}> Replay session history (${replayHistoryCount}) ${this._survivalDot('survives', t)}</label>
                    ...
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${domSnapshotCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-domSnapshot" ${cats.domSnapshot ? 'checked' : ''} ${domSnapshotCount ? '' : 'disabled'}> DOM snapshot - storage/cookies/IndexedDB (${domSnapshotCount}+) ${this._survivalDot('fresh', t)}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:9px;color:${t.cardDesc};padding:0 0 4px 24px;">Count shown is storage+cookies only - IndexedDB is read at export time (async) so it's not reflected here, but its rows (capped 50/store, truncated) are included in the actual export.</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${eventDebugLogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-eventDebugLog" ${cats.eventDebugLog ? 'checked' : ''} ${eventDebugLogCount ? '' : 'disabled'}> Event Debugger log (${eventDebugLogCount}) ${this._survivalDot('survives', t)}</label>
                    ...
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${storageWatchLogCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-storageWatchLog" ${cats.storageWatchLog ? 'checked' : ''} ${storageWatchLogCount ? '' : 'disabled'}> Storage/Cookie Watcher log (${storageWatchLogCount}) ${this._survivalDot('survives', t)}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:9px;color:${t.cardDesc};padding:0 0 4px 24px;">Log and the watches themselves both survive - unlike DOM watches, a storage/cookie watch has no live element reference, so it auto-resumes watching the same key after a reload.</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;${snapshotsCount ? '' : 'opacity:0.5;'}"><input type="checkbox" id="ttd-pi-export-snapshots" ${cats.snapshots ? 'checked' : ''} ${snapshotsCount ? '' : 'disabled'}> Snapshots (${snapshotsCount}) ${this._survivalDot('survives', t)}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;"><input type="checkbox" id="ttd-pi-export-tokenVault" ${cats.tokenVault ? 'checked' : ''}> Token Vault (${tokenVaultCount}) ${this._survivalDot('survives', t)}</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:9px;color:${t.statusBad};padding:0 0 4px 24px;">Off by default - includes raw, unredacted auth/session token values, and the vault itself now persists across reloads. Only check this if you actually want those values in whatever this export ends up.</label>
                    <label style="display:flex;align-items:center;gap:6px;font-size:11px;padding:4px 0;"><input type="checkbox" id="ttd-pi-export-storageVault" ${cats.storageVault ? 'checked' : ''}> Storage/Cookie Vault (${storageVaultCount}) ${this._survivalDot('survives', t)}</label>
            ...
            document.getElementById('ttd-pi-export-raw').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const ok = await copyToClipboard(await InvestigationExport.rawExport(this._exportCategories));
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy raw JSON'; }, 1200);
            };
            document.getElementById('ttd-pi-export-brief').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const ok = await copyToClipboard(await InvestigationExport.briefing(this._exportCategories));
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy AI briefing'; }, 1200);
            };
            document.getElementById('ttd-pi-export-csv').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const csv = await InvestigationExport.toCSV(this._exportCategories);
            ...
            document.getElementById('ttd-pi-export-raw-file').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const ok = Helpers.downloadFile(Helpers._exportFilename('txt'), await InvestigationExport.rawExport(this._exportCategories), 'text/plain');
                e.target.textContent = ok ? 'Downloaded!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Download JSON (.txt)'; }, 1200);
            };
            document.getElementById('ttd-pi-export-brief-file').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const ok = Helpers.downloadFile(Helpers._exportFilename('txt'), await InvestigationExport.briefing(this._exportCategories), 'text/plain');
                e.target.textContent = ok ? 'Downloaded!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Download briefing (.txt)'; }, 1200);
            };
            document.getElementById('ttd-pi-export-csv-file').onclick = async (e) => {
                e.target.textContent = 'Gathering...';
                const csv = await InvestigationExport.toCSV(this._exportCategories);
        ...
        _renderGlobals() {
            ...
            globals.forEach((g) => {
                const row = document.createElement('div');
                const isHighlighted = this._highlightGlobalKey && g.key === this._highlightGlobalKey;
                row.style.cssText = `padding:6px 8px;font-size:11px;border-bottom:1px solid ${t.rowBorder};word-break:break-all;${isHighlighted ? `background:${t.secondaryBtnBg};box-shadow:inset 3px 0 0 ${t.statusOk};` : ''}`;
                const isFn = g.type === 'function';
                row.innerHTML = `
                    <div style="cursor:pointer;">
                        <span style="font-weight:700;">${Helpers._escape(g.key)}</span> <span style="color:${t.cardDesc};">(${Helpers._escape(g.type)})</span>${isHighlighted ? ` <span style="color:${t.statusOk};font-size:9px;">- found via Tech Fingerprint</span>` : ''}<br>
                        <span style="font-size:10px;color:${t.cardDesc};">${Helpers._escape(g.preview)}</span>
                    </div>
                    <button data-inspect-global style="${Helpers._secondaryBtnStyle(t)}margin-top:4px;">${isFn ? 'Call' : 'Get/Set'} via Path access</button>
                `;
                if (isHighlighted) setTimeout(() => { try { row.scrollIntoView({ block: 'center', behavior: 'smooth' }); } catch {  } }, 0);
                row.querySelector('div').onclick = async () => { await copyToClipboard(g.full); };
                row.querySelector('[data-inspect-global]').onclick = (e) => {
                    e.stopPropagation();
                    this._pathAccessPath = g.key;
                    this._consoleExecMode = 'path';
                    this._pathAccessOp = isFn ? 'call' : 'get';
                    this._goToSection('console');
                };
                listEl.appendChild(row);
            });
        },
        ...
        _renderStorageDetail(store, entries) {
            ...
            if (this._addingStorageKey) {
                area.innerHTML = `
                    ...
                    <div style="display:flex;gap:6px;">
                        <button id="ttd-pi-storage-save-new" style="${Helpers._primaryBtnStyleInline()}">Save</button>
            ...
            area.innerHTML = `
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <button id="ttd-pi-storage-save" style="${Helpers._primaryBtnStyleInline()}">Save</button>
                    ...
                    <button id="ttd-pi-storage-to-vault" style="${Helpers._secondaryBtnStyle(t)}">Save to vault</button>
                </div>
            ...
            document.getElementById('ttd-pi-storage-to-vault').onclick = (e) => {
                const value = document.getElementById('ttd-pi-storage-edit-value').value;
                const label = prompt('Optional label for this saved value:', '');
                if (label === null) return;
                StorageVault.add(this._storageType, entry.key, value, label);
                e.target.textContent = 'Saved!';
                setTimeout(() => { if (e.target.isConnected) e.target.textContent = 'Save to vault'; }, 1200);
            };
        },
        ...
        _renderCookieDetail(cookies) {
            ...
            if (this._addingCookie) {
                area.innerHTML = `
                    ...
                    <div style="display:flex;gap:6px;">
                        <button id="ttd-pi-cookie-save-new" style="${Helpers._primaryBtnStyleInline()}">Save</button>
            ...
            area.innerHTML = `
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <button id="ttd-pi-cookie-save" style="${Helpers._primaryBtnStyleInline()}">Save</button>
                    ...
                    <button id="ttd-pi-cookie-to-vault" style="${Helpers._secondaryBtnStyle(t)}">Save to vault</button>
                </div>
            ...
            document.getElementById('ttd-pi-cookie-to-vault').onclick = (e) => {
                const value = document.getElementById('ttd-pi-cookie-edit-value').value;
                const label = prompt('Optional label for this saved value:', '');
                if (label === null) return;
                StorageVault.add('cookie', cookie.key, value, label);
                e.target.textContent = 'Saved!';
                setTimeout(() => { if (e.target.isConnected) e.target.textContent = 'Save to vault'; }, 1200);
            };
        },
        ...
        _renderStorageVault() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-content');
            const entries = StorageVault.all();
            const kindLabel = { local: 'localStorage', session: 'sessionStorage', cookie: 'cookie' };

            area.innerHTML = `
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:6px;">A curated set of specific values you've deliberately kept, independent of whatever's currently live - same idea as the Token Vault. Persists across reloads. Save entries from a key's detail view in localStorage/sessionStorage/Cookies.</div>
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                    <span style="font-size:10px;color:${t.cardDesc};">${entries.length} of up to ${StorageVault.MAX} saved</span>
                    ${entries.length ? `<button id="ttd-pi-storagevault-clear" style="${Helpers._secondaryBtnStyle(t)}">Clear vault</button>` : ''}
                </div>
                <div id="ttd-pi-storagevault-list" style="max-height:180px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:8px;"></div>
                <div id="ttd-pi-storagevault-detail"></div>
            `;

            const clearBtn = document.getElementById('ttd-pi-storagevault-clear');
            if (clearBtn) clearBtn.onclick = () => {
                if (!confirm('Clear all saved values from the Storage/Cookie Vault? This cannot be undone.')) return;
                StorageVault.clear();
                this._selectedStorageVaultId = null;
                this._renderStorageVault();
            };

            const listEl = document.getElementById('ttd-pi-storagevault-list');
            if (!entries.length) {
                listEl.innerHTML = `<div style="padding:8px;font-size:11px;color:${t.cardDesc};">Nothing saved yet.</div>`;
                document.getElementById('ttd-pi-storagevault-detail').innerHTML = '';
                return;
            }
            if (this._selectedStorageVaultId === undefined || this._selectedStorageVaultId === null || !entries.some((e) => e.id === this._selectedStorageVaultId)) {
                this._selectedStorageVaultId = entries[0].id;
            }

            entries.forEach((entry) => {
                const isSelected = entry.id === this._selectedStorageVaultId;
                const row = document.createElement('div');
                row.style.cssText = `padding:6px 8px;font-size:11px;cursor:pointer;border-bottom:1px solid ${t.rowBorder};background:${isSelected ? t.secondaryBtnBg : 'transparent'};word-break:break-all;`;
                row.innerHTML = `<span style="font-weight:700;color:${t.statusOk};">${kindLabel[entry.kind]}</span>: ${Helpers._escape(entry.key)} ${entry.label ? `<span style="color:${t.statusOk};">- ${Helpers._escape(entry.label)}</span>` : ''}<br><span style="font-size:10px;color:${t.cardDesc};">${Helpers._escape(entry.value.slice(0, 80))}${entry.value.length > 80 ? '...' : ''}</span>`;
                row.onclick = () => { this._selectedStorageVaultId = entry.id; this._renderStorageVaultDetail(); };
                listEl.appendChild(row);
            });

            this._renderStorageVaultDetail();
        },

        _renderStorageVaultDetail() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-storagevault-detail');
            if (!area) return;
            const entry = StorageVault.all().find((e) => e.id === this._selectedStorageVaultId);
            if (!entry) { area.innerHTML = ''; return; }
            const kindLabel = { local: 'localStorage', session: 'sessionStorage', cookie: 'cookie' };

            area.innerHTML = `
                <div style="font-size:11px;font-weight:700;margin-bottom:2px;word-break:break-all;">${kindLabel[entry.kind]}: ${Helpers._escape(entry.key)}</div>
                ${entry.label ? `<div style="font-size:10px;color:${t.statusOk};margin-bottom:4px;">${Helpers._escape(entry.label)}</div>` : ''}
                <textarea id="ttd-pi-storagevault-value" style="width:100%;box-sizing:border-box;height:100px;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;font-family:monospace;">${Helpers._escape(entry.value)}</textarea>
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Apply to key (defaults to the key it was saved from)</div>
                <input id="ttd-pi-storagevault-applykey" value="${Helpers._escape(entry.key)}" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <button id="ttd-pi-storagevault-save" style="${Helpers._secondaryBtnStyle(t)}">Save edit</button>
                    <button id="ttd-pi-storagevault-apply" style="${Helpers._primaryBtnStyleInline()}">Apply to live page</button>
                    <button id="ttd-pi-storagevault-remove" style="${Helpers._secondaryBtnStyle(t)}color:${t.statusBad};">Remove from vault</button>
                    <button id="ttd-pi-storagevault-copy" style="${Helpers._secondaryBtnStyle(t)}">Copy value</button>
                </div>
                <div id="ttd-pi-storagevault-status" style="font-size:10px;margin-top:4px;"></div>
            `;

            const statusEl = () => document.getElementById('ttd-pi-storagevault-status');

            document.getElementById('ttd-pi-storagevault-save').onclick = () => {
                const value = document.getElementById('ttd-pi-storagevault-value').value;
                StorageVault.updateValue(entry.id, value);
                const s = statusEl();
                s.textContent = 'Saved.';
                s.style.color = t.statusOk;
            };
            document.getElementById('ttd-pi-storagevault-apply').onclick = () => {
                const value = document.getElementById('ttd-pi-storagevault-value').value;
                const applyKey = document.getElementById('ttd-pi-storagevault-applykey').value;
                StorageVault.updateValue(entry.id, value);
                const result = StorageVault.apply(entry.id, applyKey);
                const s = statusEl();
                s.textContent = result.ok ? `Applied to ${kindLabel[entry.kind]}.${applyKey || entry.key} on the live page.` : result.error;
                s.style.color = result.ok ? t.statusOk : t.statusBad;
            };
            document.getElementById('ttd-pi-storagevault-remove').onclick = () => {
                StorageVault.remove(entry.id);
                this._selectedStorageVaultId = null;
                this._renderStorageVault();
            };
            document.getElementById('ttd-pi-storagevault-copy').onclick = async (e) => {
                const ok = await copyToClipboard(entry.value);
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { if (e.target.isConnected) e.target.textContent = 'Copy value'; }, 1200);
            };
        },
        ...
        _renderIndexedDBRecordDetail(store) {
            ...
            if (this._idbAddingRecord) {
                area.innerHTML = `
                    ...
                    <div style="display:flex;gap:6px;">
                        <button id="ttd-pi-idb-save-new" style="${Helpers._primaryBtnStyleInline()}">Save</button>
            ...
            area.innerHTML = `
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    ${isEditable ? `<button id="ttd-pi-idb-save" style="${Helpers._primaryBtnStyleInline()}">Save</button>` : ''}
        ...
        _renderReplayEditor() {
            ...
            panel.innerHTML = `
                ...
                <select id="ttd-replay-sweep-preset" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:4px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                    <option value="">Load payload preset (overwrites values below)...</option>
                    ${Object.entries(RequestReplay.PAYLOAD_PRESETS).map(([key, p]) => `<option value="${key}">${Helpers._escape(p.label)} (${p.values.length})</option>`).join('')}
                </select>
                <div style="font-size:9px;color:${t.cardDesc};margin-bottom:6px;">Standard SQLi/XSS/path-traversal probe strings for testing how an endpoint handles unexpected input. Review the list before running, and only against origins you're authorized to test.</div>
                <textarea id="ttd-replay-sweep-values" rows="6" placeholder="Values to sweep, comma or newline separated (e.g. 1,2,3 or a,b,c)" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;font-family:monospace;">${Helpers._escape(this._replaySweepValuesText)}</textarea>
                <div style="display:flex;gap:6px;margin-bottom:6px;">
            ...
            document.getElementById('ttd-replay-sweep-preset').onchange = (e) => this._loadSweepPreset(e.target.value);
        ...
        _loadSweepPreset(key) {
            if (!key) return;
            const preset = RequestReplay.PAYLOAD_PRESETS[key];
            if (!preset) return;
            if (this._replaySweepValuesText.trim() && !confirm(`Replace the current sweep values with the ${preset.values.length} ${preset.label} strings? This overwrites what's in the values box.`)) {
                this._renderReplayEditor();
                return;
            }
            this._replaySweepValuesText = preset.values.join('\n');
            this._renderReplayEditor();
        },
        ...
        _runReplaySweep() {
            ...
            this._replaySweepRunning = RequestReplay.runSweep(template, placeholder, values, this._replaySweepDelay, this._replaySendMode, {
                onStep: (stepIndex, value, result) => {
                    const bodyText = result.ok ? (result.rawText || (result.json ? JSON.stringify(result.json) : '') || '') : '';
                    const errorSignatures = bodyText ? Recon.ERROR_BODY_PATTERNS.filter((p) => p.re.test(bodyText)).map((p) => p.label) : [];
                    const requestDraft = RequestReplay.buildDraftForValue(template, placeholder, value);
                    this._replaySweepLog.push({ value, ok: result.ok, status: result.ok ? result.status : null, durationMs: result.durationMs, length: result.ok ? bodyText.length : null, errorSignatures, error: result.ok ? null : result.error, timestamp: Date.now(), requestDraft, result });
                    const annotated = RequestReplay.detectAnomalies(this._replaySweepLog);
                    const thisAnnotated = annotated[annotated.length - 1];
                    const historyEntry = ReplayHistory.add(requestDraft, this._replayOriginalEntry, result);
                    historyEntry.sweep = { value, placeholder, anomaly: thisAnnotated.anomaly, anomalyReasons: thisAnnotated.anomalyReasons, anomalyReasonTypes: thisAnnotated.anomalyReasonTypes, isBaseline: thisAnnotated.isBaseline };
                    this._renderReplaySweepLog();
                    this._renderReplayHistorySection();
                },
                onDone: () => {
        ...
        _renderReplaySweepLog() {
            ...
            el.innerHTML = `
                <div style="font-size:11px;font-weight:700;margin:6px 0 4px;">Sweep results (${annotated.length})${anomalyCount ? ` - <span style="color:${t.statusBad};">${anomalyCount} flagged</span>` : ''}</div>
                ${annotated.length > 1 ? `<div style="font-size:9px;color:${t.cardDesc};margin-bottom:4px;">First value sent is treated as the baseline. Later values are flagged when status, body length, or timing diverge from it, or when the body matches a known verbose-error signature - all deltas, not confirmations of a real vulnerability. Tap a row for the exact request/response.</div>` : `<div style="font-size:9px;color:${t.cardDesc};margin-bottom:4px;">Tap a row for the exact request/response.</div>`}
                <div style="max-height:220px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;">
                    ${annotated.slice().reverse().map((l, revIdx) => {
                        const idx = annotated.length - 1 - revIdx;
                        return `
                        <div data-idx="${idx}" style="padding:5px 8px;border-bottom:1px solid ${t.rowBorder};font-size:10px;cursor:pointer;${l.anomaly ? `background:${t.statusBad}1a;` : ''}">
                            <div>
                                <b>${Helpers._escape(String(l.value))}</b>${l.isBaseline ? ` <span style="color:${t.cardDesc};">(baseline)</span>` : ''}
                                - ${l.ok ? `<span style="color:${l.status >= 200 && l.status < 300 ? t.statusOk : t.statusBad};">${l.status}</span> - ${l.durationMs}ms${typeof l.length === 'number' ? ` - ${l.length}B` : ''}` : `<span style="color:${t.statusBad};">${Helpers._escape(l.error)}</span>`}
            ...
            el.querySelectorAll('[data-idx]').forEach((row) => {
                row.onclick = () => this._openSweepResultDetail(Number(row.getAttribute('data-idx')), annotated);
            });
        },

        _openSweepResultDetail(idx, annotated) {
            const entry = annotated[idx];
            if (!entry) return;
            this._renderPayloadResultDetail({
                titleSuffix: entry.isBaseline ? ' (baseline)' : '',
                value: entry.value,
                anomaly: entry.anomaly,
                anomalyReasons: entry.anomalyReasons,
                draft: entry.requestDraft,
                result: entry.result
            });
        },

        _openReplayHistoryFindingDetail(historyEntry) {
            if (!historyEntry) return;
            const sweep = historyEntry.sweep || {};
            this._renderPayloadResultDetail({
                titleSuffix: sweep.isBaseline ? ' (baseline)' : '',
                value: sweep.value,
                anomaly: sweep.anomaly,
                anomalyReasons: sweep.anomalyReasons,
                draft: historyEntry.draft,
                result: historyEntry.result
            });
        },

        _renderPayloadResultDetail(opts) {
            const t = Theme.palette;
            document.getElementById('ttd-sweep-detail-panel')?.remove();
            const r = opts.result || {};
            const draft = opts.draft || {};
            const bodyText = r.ok ? (r.rawText || (r.json ? JSON.stringify(r.json) : '') || '') : '';
            const pretty = r.json ? JSON.stringify(r.json, null, 2) : (r.rawText || (r.ok ? '(empty body)' : ''));
            const statusColor = r.ok && r.status >= 200 && r.status < 300 ? t.statusOk : t.statusBad;
            const reqHeaders = (draft.headersText || '').trim() || '(none set explicitly)';
            const resHeaders = r.responseHeaders && Object.keys(r.responseHeaders).length
                ? Object.keys(r.responseHeaders).map((k) => `${k}: ${r.responseHeaders[k]}`).join('\n')
                : '(none captured)';

            const panel = document.createElement('div');
            panel.id = 'ttd-sweep-detail-panel';
            panel.style.cssText = `position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:320px;max-width:92vw;max-height:85vh;overflow-y:auto;background:${t.panelBg};color:${t.panelText};border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.6);z-index:1000000;padding:16px;font-size:13px;`;
            panel.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="font-size:14px;font-weight:700;">Sweep result${opts.titleSuffix || ''}</span>
                    <button id="ttd-sweep-detail-close" style="background:none;border:none;color:${t.panelText};cursor:pointer;font-size:20px;line-height:1;">\u00D7</button>
                </div>
                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Payload value swept in</div>
                <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:8px;max-height:80px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:11px;margin:0 0 8px;">${Helpers._escape(String(opts.value))}</pre>

                ${opts.anomaly ? `
                <div style="font-size:11px;font-weight:700;color:${t.statusBad};margin-bottom:2px;">Flagged</div>
                <div style="font-size:10px;color:${t.statusBad};margin-bottom:8px;">${(opts.anomalyReasons || []).map((r2) => `- ${Helpers._escape(r2)}`).join('<br>')}</div>
                ` : ''}

                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Request sent</div>
                <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:8px;max-height:100px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:10px;margin:0 0 4px;">${Helpers._escape(draft.method || '')} ${Helpers._escape(draft.url || '')}</pre>
                <div style="margin:4px 0;">
                    <span id="ttd-sweep-detail-headers-toggle" style="font-size:10px;text-decoration:underline;cursor:pointer;color:${t.cardDesc};">Show request/response headers</span>
                    <div id="ttd-sweep-detail-headers-body" style="display:none;margin-top:4px;">
                        <div style="font-size:10px;font-weight:700;margin-bottom:2px;">Request headers</div>
                        <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:6px;max-height:100px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:10px;margin:0 0 6px;">${Helpers._escape(reqHeaders)}</pre>
                        <div style="font-size:10px;font-weight:700;margin-bottom:2px;">Response headers</div>
                        <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:6px;max-height:100px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:10px;margin:0;">${Helpers._escape(resHeaders)}</pre>
                    </div>
                </div>
                ${draft.body ? `
                <div style="font-size:10px;color:${t.cardDesc};margin:6px 0 2px;">Request body</div>
                <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:8px;max-height:100px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:10px;margin:0 0 8px;">${Helpers._escape(draft.body)}</pre>
                ` : ''}

                <div style="font-size:11px;font-weight:700;margin-bottom:2px;">Response</div>
                <div style="font-size:11px;color:${statusColor};font-weight:700;margin-bottom:6px;">${r.ok ? `${r.status} ${Helpers._escape(r.statusText || '')} - ${r.durationMs}ms - ${bodyText.length}B` : `failed - ${Helpers._escape(r.error || 'unknown error')}`}</div>
                <pre style="background:${t.secondaryBtnBg};border:1px solid ${t.rowBorder};border-radius:6px;padding:8px;max-height:200px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:10px;margin:0 0 8px;">${Helpers._escape(pretty || '(empty)')}</pre>
                <button id="ttd-sweep-detail-copy" style="${Helpers._secondaryBtnStyle(t)}width:100%;">Copy response body</button>
            `;
            document.body.appendChild(panel);

            document.getElementById('ttd-sweep-detail-close').onclick = () => panel.remove();
            document.getElementById('ttd-sweep-detail-headers-toggle').onclick = (e) => {
                const body = document.getElementById('ttd-sweep-detail-headers-body');
                const show = body.style.display === 'none';
                body.style.display = show ? '' : 'none';
                e.target.textContent = show ? 'Hide request/response headers' : 'Show request/response headers';
            };
            document.getElementById('ttd-sweep-detail-copy').onclick = async (e) => {
                const ok = await copyToClipboard(pretty || '');
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy response body'; }, 1200);
            };
        },
        ...
        _renderReplayHistorySection() {
            ...
            const rows = entries.map((entry, i) => {
                ...
                const sweepTag = entry.sweep
                    ? ` <span style="color:${entry.sweep.anomaly ? t.statusBad : t.cardDesc};">${entry.sweep.isBaseline ? '(sweep baseline)' : entry.sweep.anomaly ? `(sweep - flagged: ${Helpers._escape(String(entry.sweep.value))})` : `(sweep: ${Helpers._escape(String(entry.sweep.value))})`}</span>`
                    : '';
                return `
                    <div data-idx="${i}" style="padding:6px 8px;font-size:10px;cursor:pointer;border-bottom:1px solid ${t.rowBorder};word-break:break-all;${entry.sweep && entry.sweep.anomaly ? `background:${t.statusBad}1a;` : ''}">
                        <b>${Helpers._escape(entry.draft.method)}</b> ${Helpers._escape(Helpers._shortenUrl(entry.draft.url))}${sweepTag}<br>
        ...
        _severityColor(t, severity) {
            if (severity === 'high') return t.statusBad;
            if (severity === 'medium') return t.statusWarn;
            if (severity === 'low') return t.statusNeutral;
            return t.statusOk;
        },

        _navigateToLocate(locate) {
            if (!locate) return;
            if (locate.kind === 'script') {
                const scripts = PageInspector.getInlineScripts();
                const match = scripts.find((s) => s.external && s.src === locate.src);
                this._scriptsSnapshot = scripts;
                this._expandedScriptIndex = match ? match.index : null;
                this._goToSection('scripts');
                return;
            }
            if (locate.kind === 'global') {
                this._globalsSnapshot = PageInspector.getExtraWindowGlobals();
                this._highlightGlobalKey = locate.key;
                this._goToSection('globals');
                return;
            }
            if (locate.kind === 'cookie') {
                this._addingCookie = false;
                this._selectedCookieKey = locate.key;
                this._goToSection('cookies');
                return;
            }
            if (locate.kind === 'traffic') {
                this._trafficSearch = Helpers._shortenUrl(locate.url).split('?')[0];
                this._goToSection('traffic');
                return;
            }
            if (locate.kind === 'headers') {
                this._trafficSearch = location.host;
                this._goToSection('traffic');
                return;
            }
            if (locate.kind === 'replayHistory') {
                const entry = ReplayHistory.get(locate.historyEntryId);
                if (entry) this._openReplayHistoryFindingDetail(entry);
                return;
            }
            this._goToSection('dom');
        },
        ...
        _renderFingerprint() {
            ...
            area.innerHTML = `
                ...
                <div id="ttd-pi-fp-list" style="border:1px solid ${t.rowBorder};border-radius:6px;overflow:hidden;"></div>
            `;

            const listEl = document.getElementById('ttd-pi-fp-list');
            if (!hits.length) {
                listEl.style.border = 'none';
                listEl.innerHTML = `<div style="font-size:11px;color:${t.cardDesc};">No known signatures matched yet.</div>`;
            } else {
                hits.forEach((h) => {
                    const row = document.createElement('div');
            ...
            document.getElementById('ttd-pi-fp-rescan').onclick = (e) => {
                Recon.fingerprint(true);
                this._renderFingerprint();
                const btn = document.getElementById('ttd-pi-fp-rescan');
                if (btn) {
                    const original = 'Re-scan';
                    btn.textContent = 'Rescanned!';
                    setTimeout(() => { if (btn.isConnected) btn.textContent = original; }, 1200);
                }
            };
            document.getElementById('ttd-pi-fp-copy').onclick = async (e) => {
                const ok = await copyToClipboard(JSON.stringify(hits, null, 2));
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy all'; }, 1200);
            };
        },
        ...
        _renderMisconfig() {
            ...
            area.innerHTML = `
                ...
                <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;">
                    ...
                    <button id="ttd-pi-mc-copy" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Copy all</button>
                </div>
                <div id="ttd-pi-mc-headers-status" style="font-size:10px;margin-bottom:8px;">
                    ${Recon._pageHeadersError ? `<span style="color:${t.statusBad};">Header fetch: ${Helpers._escape(Recon._pageHeadersError)}</span>` : ''}
                    ${(!Recon._pageHeadersError && Recon._pageHeadersFetchedAt) ? `<span style="color:${t.statusOk};">Headers last checked ${Helpers._timeAgo(Recon._pageHeadersFetchedAt)} - ${Object.keys(Recon._pageHeaders || {}).length} header(s) found.</span>` : ''}
                </div>
                <div id="ttd-pi-mc-list" style="border:1px solid ${t.rowBorder};border-radius:6px;overflow:hidden;"></div>
            `;

            const listEl = document.getElementById('ttd-pi-mc-list');
            if (!findings.length) {
                listEl.style.border = 'none';
                listEl.innerHTML = `<div style="font-size:11px;color:${t.cardDesc};">No findings.</div>`;
            } else {
                findings.forEach((f) => {
                    const row = document.createElement('div');
                    const clickable = !!f.locate;
                    row.style.cssText = `padding:6px 8px;font-size:11px;border-bottom:1px solid ${t.rowBorder};word-break:break-word;${clickable ? 'cursor:pointer;' : ''}`;
                    row.innerHTML = `
                        <span style="color:${this._severityColor(t, f.severity)};font-weight:700;text-transform:uppercase;font-size:10px;">${f.severity}</span>
                        <span style="font-weight:700;"> ${Helpers._escape(f.label)}</span>
                        <br><span style="font-size:10px;color:${t.cardDesc};">${Helpers._escape(f.detail)}${clickable ? ` <span style="color:${t.statusOk};">- tap to view</span>` : ''}</span>
                    `;
                    if (clickable) row.onclick = () => this._navigateToLocate(f.locate);
                    listEl.appendChild(row);
                });
            }

            document.getElementById('ttd-pi-mc-fetchheaders').onclick = async (e) => {
                Recon._pageHeadersFetching = true;
                this._renderMisconfig();
                await Recon.fetchPageHeaders();
                Recon.misconfigAudit(true);
                this._renderMisconfig();
            };
            document.getElementById('ttd-pi-mc-rescan').onclick = () => {
                Recon.misconfigAudit(true);
                this._renderMisconfig();
                const btn = document.getElementById('ttd-pi-mc-rescan');
                if (btn) {
                    const original = 'Re-scan';
                    btn.textContent = 'Rescanned!';
                    setTimeout(() => { if (btn.isConnected) btn.textContent = original; }, 1200);
            ...
            document.getElementById('ttd-pi-mc-copy').onclick = async (e) => {
                const ok = await copyToClipboard(JSON.stringify(findings, null, 2));
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy all'; }, 1200);
            };
        },
        ...
        _renderFindings() {
            ...
            area.innerHTML = `
                ...
                <div style="font-size:11px;margin-bottom:8px;">
                    ...
                    <span style="color:${t.statusNeutral};font-weight:700;"> ${counts.low || 0} low</span>
                    <span style="color:${t.cardDesc};"> - ${findings.length} total</span>
                </div>
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <button id="ttd-pi-findings-rescan" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Re-scan</button>
                    <button id="ttd-pi-findings-copy" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Copy all</button>
                </div>
                <div id="ttd-pi-findings-list" style="border:1px solid ${t.rowBorder};border-radius:6px;overflow:hidden;"></div>
            `;

            const listEl = document.getElementById('ttd-pi-findings-list');
            if (!findings.length) {
                listEl.style.border = 'none';
                listEl.innerHTML = `<div style="font-size:11px;color:${t.cardDesc};">No findings yet - run a Tech Fingerprint / Misconfig scan, or a Parameter Sweep, then re-scan here.</div>`;
            } else {
                findings.forEach((f) => {
                    const row = document.createElement('div');
                    const clickable = !!f.locate;
                    row.style.cssText = `padding:6px 8px;font-size:11px;border-bottom:1px solid ${t.rowBorder};word-break:break-word;${clickable ? 'cursor:pointer;' : ''}`;
                    row.innerHTML = `
                        <span style="color:${this._severityColor(t, f.confidence)};font-weight:700;text-transform:uppercase;font-size:10px;">${f.confidence}</span>
                        <span style="color:${t.cardDesc};font-size:10px;"> ${f.score}/100</span>
                        <span style="color:${t.cardDesc};font-size:10px;"> - ${sourceLabel[f.source] || f.source}</span>
                        <br><span style="font-weight:700;">${Helpers._escape(f.label)}</span>
                        <br><span style="font-size:10px;color:${t.cardDesc};">${Helpers._escape(f.detail)}${clickable ? ` <span style="color:${t.statusOk};">- tap to view</span>` : ''}</span>
                    `;
                    if (clickable) row.onclick = () => this._navigateToLocate(f.locate);
                    listEl.appendChild(row);
            ...
            document.getElementById('ttd-pi-findings-rescan').onclick = () => {
                Recon.misconfigAudit(true);
                Recon.fingerprint(true);
                Findings.gather(true);
                this._renderFindings();
                const btn = document.getElementById('ttd-pi-findings-rescan');
                if (btn) {
                    const original = 'Re-scan';
                    btn.textContent = 'Rescanned!';
                    setTimeout(() => { if (btn.isConnected) btn.textContent = original; }, 1200);
            ...
            document.getElementById('ttd-pi-findings-copy').onclick = async (e) => {
                const ok = await copyToClipboard(JSON.stringify(findings, null, 2));
                e.target.textContent = ok ? 'Copied!' : 'Failed';
                setTimeout(() => { e.target.textContent = 'Copy all'; }, 1200);
            };
        ...
        _renderPathDiscovery() {
            const t = Theme.palette;
            const area = document.getElementById('ttd-pi-content');
            if (!this._discoveryBaseUrl) this._discoveryBaseUrl = location.origin;
            if (!this._discoveryWordlistText) this._discoveryWordlistText = ActiveDiscovery.DEFAULT_WORDLIST.join('\n');
            const parsedCount = ActiveDiscovery.parseWordlist(this._discoveryWordlistText).length;

            area.innerHTML = `
                <div style="font-size:10px;color:${t.statusWarn};margin-bottom:8px;">Sends one real GET request per path in the list below, against the live server, using this page's current session/cookies. Each attempt is a real request - nothing here is passive.</div>

                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Send mode</div>
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <button id="ttd-disc-mode-standard" style="${Helpers._pillStyle(t, this._discoveryMode !== 'privileged')}flex:1;">Standard (fetch)</button>
                    <button id="ttd-disc-mode-privileged" style="${Helpers._pillStyle(t, this._discoveryMode === 'privileged')}flex:1;" ${RequestReplay.privilegedAvailable() ? '' : 'disabled'}>Privileged (GM)</button>
                </div>

                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Base URL</div>
                <input id="ttd-disc-baseurl" type="text" value="${Helpers._escape(this._discoveryBaseUrl)}" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:8px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;word-break:break-all;">

                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px;">
                    <span style="font-size:10px;color:${t.cardDesc};">Paths (<span id="ttd-disc-parsed-count">${parsedCount}</span> parsed) - one per line, or CSV (first column of each row is used)</span>
                </div>
                <textarea id="ttd-disc-wordlist" rows="6" style="width:100%;box-sizing:border-box;padding:6px;margin-bottom:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:10px;font-family:monospace;">${Helpers._escape(this._discoveryWordlistText)}</textarea>
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <button id="ttd-disc-import-csv" style="${Helpers._secondaryBtnStyle(t)}flex:1;">Import CSV/TXT file</button>
                    <button id="ttd-disc-reset-wordlist" style="${Helpers._secondaryBtnStyle(t)}flex:1;">Reset to default (${ActiveDiscovery.DEFAULT_WORDLIST.length})</button>
                </div>
                ...
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <input id="ttd-disc-save-name" type="text" placeholder="Name this list, e.g. Partner A" value="${Helpers._escape(this._discoverySaveNameInput)}" style="flex:1;min-width:0;box-sizing:border-box;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                    <button id="ttd-disc-save-list" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Save current as...</button>
                </div>

                <div style="display:flex;gap:6px;margin-bottom:8px;align-items:center;">
                    <span style="font-size:10px;color:${t.cardDesc};">delay (ms)</span>
                    <input id="ttd-disc-delay" type="number" min="${ActiveDiscovery.MIN_DELAY_MS}" step="50" value="${this._discoveryDelay}" style="width:80px;box-sizing:border-box;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                </div>
            ...
            if (privBtn && !privBtn.disabled) privBtn.onclick = () => { this._discoveryMode = 'privileged'; this._renderPathDiscovery(); };
            document.getElementById('ttd-disc-baseurl').oninput = (e) => { this._discoveryBaseUrl = e.target.value; };
            document.getElementById('ttd-disc-wordlist').oninput = (e) => {
                this._discoveryWordlistText = e.target.value;
                const countEl = document.getElementById('ttd-disc-parsed-count');
                if (countEl) countEl.textContent = ActiveDiscovery.parseWordlist(this._discoveryWordlistText).length;
            };
            ...
            if (stopBtn) stopBtn.onclick = () => {
                if (this._discoveryRunning) this._discoveryRunning.stop();
                this._discoveryRunning = null;
                this._renderPathDiscovery();
            };

            document.getElementById('ttd-disc-save-name').oninput = (e) => { this._discoverySaveNameInput = e.target.value; };
            document.getElementById('ttd-disc-save-list').onclick = () => {
                const name = (this._discoverySaveNameInput || '').trim();
                if (!name) return;
                const existing = ActiveDiscovery.loadSavedWordlists();
                if (existing[name] && !confirm(`"${name}" already exists - overwrite it with the current list?`)) return;
                ActiveDiscovery.saveWordlist(name, this._discoveryWordlistText);
                this._discoverySaveNameInput = '';
                this._renderPathDiscovery();
            };

            const importBtn = document.getElementById('ttd-disc-import-csv');
            const importInput = document.getElementById('ttd-disc-import-input');
            ...
            importInput.onchange = () => {
                ...
                reader.onload = () => {
                    this._discoveryWordlistText = String(reader.result || '');
                    this._renderPathDiscovery();
                    const status = document.getElementById('ttd-disc-import-status');
                    if (status) status.innerHTML = `<span style="color:${t.statusOk};">Imported ${Helpers._escape(file.name)} - ${ActiveDiscovery.parseWordlist(this._discoveryWordlistText).length} paths parsed. Use "Save current as..." below to keep it as a named list.</span>`;
                };
                reader.onerror = () => {
                    const status = document.getElementById('ttd-disc-import-status');
                    if (status) status.innerHTML = `<span style="color:${t.statusBad};">Couldn't read ${Helpers._escape(file.name)}.</span>`;
                };
        ...
        _renderDiscoverySavedLists() {
            const t = Theme.palette;
            const el = document.getElementById('ttd-disc-saved-lists');
            if (!el) return;
            const saved = ActiveDiscovery.loadSavedWordlists();
            const names = Object.keys(saved);
            if (!names.length) {
                el.innerHTML = `<div style="font-size:10px;color:${t.cardDesc};">No saved lists yet - type or import a list above, then "Save current as..." to keep it.</div>`;
                return;
            }
            el.innerHTML = '';
            names.forEach((name) => {
                ...
                const count = ActiveDiscovery.parseWordlist(saved[name]).length;
                row.innerHTML = `
                    <button data-load style="${Helpers._secondaryBtnStyle(t)}flex:1;text-align:left;">${Helpers._escape(name)} <span style="color:${t.cardDesc};">(${count})</span></button>
                    <button data-delete style="${Helpers._secondaryBtnStyle(t)}color:${t.statusBad};padding:4px 8px;">Delete</button>
                `;
                row.querySelector('[data-load]').onclick = () => {
                    this._discoveryWordlistText = saved[name];
                    this._renderPathDiscovery();
                };
                row.querySelector('[data-delete]').onclick = () => {
                    if (!confirm(`Delete the saved list "${name}"? This can't be undone.`)) return;
                    ActiveDiscovery.deleteWordlist(name);
                    this._renderDiscoverySavedLists();
                };
                el.appendChild(row);
            });
        ...
        _runPathDiscovery() {
            ...
            const baseUrl = (this._discoveryBaseUrl || '').trim();
            if (!baseUrl) return;
        ...
        _renderDiscoveryLog() {
            const t = Theme.palette;
            const el = document.getElementById('ttd-disc-log');
            if (!el) return;
        ...
        _renderCrawler() {
            ...
            area.innerHTML = `
                <div style="font-size:10px;color:${t.statusWarn};margin-bottom:8px;">Crawling fetches whichever pages you select below and looks at their same-origin links - real requests, same as any other send from this tool. This never auto-follows links it finds: each round is something you review and choose to run yourself. That matters here specifically because some games (this one may be one of them) put real actions - attack, buy, use item, spend energy - behind plain links, so don't select-all blindly on a page you haven't looked at.</div>

                <button id="ttd-crawl-scan-page" style="${Helpers._secondaryBtnStyle(t)}width:100%;margin-bottom:8px;">Scan this page for links (no requests sent)</button>

                <div style="font-size:10px;color:${t.cardDesc};margin-bottom:2px;">Send mode</div>
                <div style="display:flex;gap:6px;margin-bottom:8px;">
                    <button id="ttd-crawl-mode-standard" style="${Helpers._pillStyle(t, this._crawlMode !== 'privileged')}flex:1;">Standard (fetch)</button>
                    <button id="ttd-crawl-mode-privileged" style="${Helpers._pillStyle(t, this._crawlMode === 'privileged')}flex:1;" ${RequestReplay.privilegedAvailable() ? '' : 'disabled'}>Privileged (GM)</button>
                </div>

                <div id="ttd-crawl-candidates"></div>

                <div style="display:flex;gap:6px;margin:8px 0;align-items:center;">
                    <span style="font-size:10px;color:${t.cardDesc};">delay (ms)</span>
                    <input id="ttd-crawl-delay" type="number" min="${ActiveDiscovery.MIN_DELAY_MS}" step="50" value="${this._crawlDelay}" style="width:80px;box-sizing:border-box;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:11px;">
                </div>
                <div style="display:flex;gap:6px;margin-bottom:6px;">
                    <button id="ttd-crawl-run" style="${Helpers._primaryBtnStyle()}flex:1;" ${this._crawlRunning ? 'disabled' : ''}>${this._crawlRunning ? 'Running...' : 'Crawl selected'}</button>
                    ${this._crawlRunning ? `<button id="ttd-crawl-stop" style="${Helpers._secondaryBtnStyle(t)}color:${t.statusBad};">Stop</button>` : ''}
                </div>

                <div id="ttd-crawl-log"></div>
                <div id="ttd-crawl-newfound"></div>
            `;

            document.getElementById('ttd-crawl-scan-page').onclick = () => {
                const found = ActiveDiscovery.collectLinksFromCurrentPage();
                this._addCrawlCandidates(found);
                this._renderCrawler();
            };
            document.getElementById('ttd-crawl-mode-standard').onclick = () => { this._crawlMode = 'standard'; this._renderCrawler(); };
            const privBtn = document.getElementById('ttd-crawl-mode-privileged');
            if (privBtn && !privBtn.disabled) privBtn.onclick = () => { this._crawlMode = 'privileged'; this._renderCrawler(); };
            document.getElementById('ttd-crawl-delay').oninput = (e) => { this._crawlDelay = e.target.value; };
            document.getElementById('ttd-crawl-run').onclick = () => this._runCrawl();
            const stopBtn = document.getElementById('ttd-crawl-stop');
            if (stopBtn) stopBtn.onclick = () => {
                if (this._crawlRunning) this._crawlRunning.stop();
                this._crawlRunning = null;
                this._renderCrawler();
            };

            this._renderCrawlCandidates();
            this._renderCrawlLog();
            this._renderCrawlNewFound();
        },

        _addCrawlCandidates(urls) {
            const known = new Set([...this._crawlVisited, ...this._crawlCandidates.map((c) => c.url)]);
            urls.forEach((u) => {
                if (known.has(u)) return;
                known.add(u);
                this._crawlCandidates.push({ url: u, selected: false });
            });
        },

        _sendUrlsToCrawler(urls) {
            const list = Array.from(new Set((urls || []).filter(Boolean)));
            if (!list.length) return 0;
            const before = this._crawlCandidates.length;
        ...
        _renderCrawlCandidates() {
            const t = Theme.palette;
            const el = document.getElementById('ttd-crawl-candidates');
            if (!el) return;
            if (!this._crawlCandidates.length) {
                el.innerHTML = `<div style="font-size:11px;color:${t.cardDesc};margin-bottom:6px;">No candidate links yet - scan this page, or add links found during a crawl round below.</div>`;
                return;
            }
            const selectedCount = this._crawlCandidates.filter((c) => c.selected).length;
            el.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    <span style="font-size:10px;color:${t.cardDesc};">${this._crawlCandidates.length} candidate${this._crawlCandidates.length === 1 ? '' : 's'}, ${selectedCount} selected</span>
                    <div style="display:flex;gap:6px;">
                        <button id="ttd-crawl-select-all" style="${Helpers._secondaryBtnStyle(t)}padding:2px 8px;">All</button>
                        <button id="ttd-crawl-select-none" style="${Helpers._secondaryBtnStyle(t)}padding:2px 8px;">None</button>
                        <button id="ttd-crawl-clear" style="${Helpers._secondaryBtnStyle(t)}padding:2px 8px;">Clear</button>
                    </div>
                </div>
                <div id="ttd-crawl-candidates-list" style="max-height:150px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:6px;"></div>
            `;
            const listEl = document.getElementById('ttd-crawl-candidates-list');
            this._crawlCandidates.forEach((c, i) => {
                const row = document.createElement('label');
                row.style.cssText = `display:flex;align-items:flex-start;gap:6px;padding:5px 8px;font-size:10px;border-bottom:1px solid ${t.rowBorder};word-break:break-all;cursor:pointer;`;
                row.innerHTML = `<input type="checkbox" ${c.selected ? 'checked' : ''} style="margin-top:2px;"> <span>${Helpers._escape(c.url)}</span>`;
                row.querySelector('input').onchange = (e) => { c.selected = e.target.checked; this._renderCrawlCandidates(); };
                listEl.appendChild(row);
            });
            document.getElementById('ttd-crawl-select-all').onclick = () => { this._crawlCandidates.forEach((c) => { c.selected = true; }); this._renderCrawlCandidates(); };
            document.getElementById('ttd-crawl-select-none').onclick = () => { this._crawlCandidates.forEach((c) => { c.selected = false; }); this._renderCrawlCandidates(); };
            document.getElementById('ttd-crawl-clear').onclick = () => { this._crawlCandidates = []; this._renderCrawlCandidates(); };
        },

        _runCrawl() {
            const selected = this._crawlCandidates.filter((c) => c.selected).map((c) => c.url);
            if (!selected.length) return;
            const delay = Math.max(ActiveDiscovery.MIN_DELAY_MS, Number(this._crawlDelay) || 0);
            const modeLabel = this._crawlMode === 'privileged' ? 'Privileged (GM_xmlhttpRequest)' : 'Standard (fetch)';
            if (!confirm(`Crawl ${selected.length} selected URL${selected.length === 1 ? '' : 's'}? This sends ${selected.length} real GET request${selected.length === 1 ? '' : 's'} using ${modeLabel} send, ${delay}ms apart, against your live session. If any of these links perform an action rather than just loading a page, that action will happen. Only proceed if you've reviewed the list.`)) return;

            this._crawlLog = [];
            this._crawlNewFound = [];
            this._crawlRunning = ActiveDiscovery.runCrawl(selected, delay, this._crawlMode, {
                onStep: (stepIndex, url, result, newLinks) => {
                    this._crawlVisited.push(url);
                    this._crawlCandidates = this._crawlCandidates.filter((c) => c.url !== url);
                    const html = (result.ok && !result.json && result.rawText) ? result.rawText.slice(0, 300000) : null;
                    this._crawlLog.push({ url, ok: result.ok, status: result.ok ? result.status : null, durationMs: result.durationMs, error: result.ok ? null : result.error, timestamp: Date.now(), html });
                    const known = new Set([...this._crawlVisited, ...this._crawlCandidates.map((c) => c.url), ...this._crawlNewFound]);
                    newLinks.forEach((u) => { if (!known.has(u)) { known.add(u); this._crawlNewFound.push(u); } });
                    this._renderCrawlLog();
                    this._renderCrawlNewFound();
                },
                onDone: () => {
        ...
        _inspectCrawledPage(entry) {
            if (!entry.html) return;
            const source = { url: entry.url, fetchedAt: entry.timestamp };
            this._domSnapshot = entry.html;
            this._domSnapshotSource = source;
            this._scriptsSnapshot = ActiveDiscovery.extractScriptsFromHtml(entry.html, entry.url);
            this._scriptsSnapshotSource = source;
            this._expandedScriptIndex = null;
            this._goToSection('dom');
        },

        _renderCrawlLog() {
            const t = Theme.palette;
            const el = document.getElementById('ttd-crawl-log');
            if (!el) return;
            if (!this._crawlLog.length) { el.innerHTML = ''; return; }
            el.innerHTML = `
                <div style="font-size:11px;font-weight:700;margin-bottom:4px;">Crawl results (${this._crawlLog.length})</div>
                <div id="ttd-crawl-log-list" style="max-height:220px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:8px;"></div>
            `;
            const listEl = document.getElementById('ttd-crawl-log-list');
            this._crawlLog.slice().reverse().forEach((l) => {
                const row = document.createElement('div');
                row.style.cssText = `padding:5px 8px;border-bottom:1px solid ${t.rowBorder};font-size:10px;word-break:break-all;`;
                row.innerHTML = `
                    <div>${l.ok ? `<span style="color:${l.status >= 200 && l.status < 300 ? t.statusOk : t.statusBad};">${l.status}</span> - ${l.durationMs}ms` : `<span style="color:${t.statusBad};">${Helpers._escape(l.error)}</span>`} - ${Helpers._escape(l.url)}</div>
                    ${l.html ? `<button data-inspect style="${Helpers._secondaryBtnStyle(t)}margin-top:4px;padding:2px 8px;">Inspect DOM/Scripts</button>` : ''}
                `;
                const inspectBtn = row.querySelector('[data-inspect]');
                if (inspectBtn) inspectBtn.onclick = () => this._inspectCrawledPage(l);
                listEl.appendChild(row);
            });
        },

        _renderCrawlNewFound() {
            const t = Theme.palette;
            const el = document.getElementById('ttd-crawl-newfound');
            if (!el) return;
            if (!this._crawlNewFound.length) { el.innerHTML = ''; return; }
            el.innerHTML = `
                <div style="font-size:11px;font-weight:700;margin-bottom:4px;">New links found this round (${this._crawlNewFound.length}) - not queued yet</div>
                <div style="max-height:150px;overflow-y:auto;border:1px solid ${t.rowBorder};border-radius:6px;margin-bottom:6px;">
                    ${this._crawlNewFound.map((u) => `<div style="padding:5px 8px;border-bottom:1px solid ${t.rowBorder};font-size:10px;word-break:break-all;">${Helpers._escape(u)}</div>`).join('')}
                </div>
                <button id="ttd-crawl-add-newfound" style="${Helpers._secondaryBtnStyle(t)}width:100%;">Add these to candidates for next round</button>
            `;
            document.getElementById('ttd-crawl-add-newfound').onclick = () => {
                this._addCrawlCandidates(this._crawlNewFound);
                this._crawlNewFound = [];
                this._renderCrawler();
            };
        },
        ...
        _renderEndpoints() {
            ...
            area.innerHTML = `
                ...
                <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px;">
                    ...
                    <button id="ttd-pi-endpoints-sendcrawler" style="${Helpers._secondaryBtnStyle(t)}white-space:nowrap;">Send filtered to Crawler</button>
            ...
            document.getElementById('ttd-pi-endpoints-sendcrawler').onclick = () => this._sendFilteredEndpointsToCrawler();
        ...
        _renderEndpointDetail(filtered) {
            ...
            el.innerHTML = `
                ...
                <button id="ttd-pi-endpoint-sendcrawler" style="${Helpers._secondaryBtnStyle(t)}margin-bottom:6px;">Send to Crawler</button>
            ...
            const sendCrawlerBtn = document.getElementById('ttd-pi-endpoint-sendcrawler');
            if (sendCrawlerBtn) sendCrawlerBtn.onclick = () => this._sendUrlsToCrawler([c.exampleUrl]);
        ...
        _renderScripts() {
            ...
            area.innerHTML = `
                ${this._scriptsSnapshotSource ? `<div style="font-size:10px;color:${t.statusWarn};margin-bottom:6px;word-break:break-all;">Viewing scripts from a crawled page, not the live one: ${Helpers._escape(this._scriptsSnapshotSource.url)} (fetched ${Helpers._timeAgo(this._scriptsSnapshotSource.fetchedAt)}) - <span id="ttd-pi-scripts-back-live" style="text-decoration:underline;cursor:pointer;">back to live page</span></div>` : ''}
                <div style="${Helpers._cardStyle(t)}margin-bottom:8px;">
            ...
            const backLive = document.getElementById('ttd-pi-scripts-back-live');
            if (backLive) backLive.onclick = () => { this._scriptsSnapshot = PageInspector.getInlineScripts(); this._scriptsSnapshotSource = null; this._renderScripts(); };
            ...
            document.getElementById('ttd-pi-scripts-refresh').onclick = () => { this._scriptsSnapshot = PageInspector.getInlineScripts(); this._scriptsSnapshotSource = null; this._renderScripts(); };
        ...
        _renderConsoleInputArea() {
            ...
            el.innerHTML = `
                ...
                <div style="display:flex;gap:6px;margin-top:6px;">
                    <button id="ttd-pi-console-run" style="${Helpers._primaryBtnStyleInline()}">Run</button>
    ...
    const SettingsUI = {
        show() {
            ...
            panel.style.cssText = `
                ...
                width:280px; max-width:88vw; max-height:80vh; overflow-y:auto; background:${t.panelBg}; color:${t.panelText};
            ...
            panel.innerHTML = `
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <div style="display:flex;gap:6px;align-items:center;">
                        ...
                        <span style="font-size:10px;color:${t.cardDesc};">bytes - applied independently to the DOM Mutation Watcher log, WebSocket message catalog, WebSocket connection history, Storage/Cookie Watcher log, and Event Debugger log (worst case ~5x this number combined)</span>
                    </div>
                </div>
                <div style="margin-bottom:12px;">
                    <div style="font-size:11px;color:${t.cardDesc};margin-bottom:4px;">Snapshots budget</div>
                    <div style="display:flex;gap:6px;align-items:center;">
                        <input id="ttd-settings-snapshots-budget" type="number" min="1000" step="1000" value="${Config.snapshotsPersistBudgetBytes}" style="width:90px;box-sizing:border-box;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:12px;">
                        <span style="font-size:10px;color:${t.cardDesc};">bytes - snapshots are evicted whole (oldest first) when over budget, never trimmed internally</span>
                    </div>
                </div>
                <div style="margin-bottom:12px;">
                    <div style="font-size:11px;color:${t.cardDesc};margin-bottom:4px;">Replay history budget</div>
                    <div style="display:flex;gap:6px;align-items:center;">
                        <input id="ttd-settings-replay-history-budget" type="number" min="1000" step="1000" value="${Config.replayHistoryBudgetBytes}" style="width:90px;box-sizing:border-box;padding:6px;background:${t.selectBg};color:${t.selectText};border:1px solid ${t.selectBorder};border-radius:6px;font-size:12px;">
                        <span style="font-size:10px;color:${t.cardDesc};">bytes - Replay/Edit history can carry full response bodies per entry</span>
                    </div>
                </div>
                <div style="margin-bottom:12px;">
                    ...
                    <button id="ttd-settings-clear-captured" style="${Helpers._secondaryBtnStyle(t)}width:100%;margin-bottom:6px;">Clear traffic + persisted cache + recorder + snapshots + watch logs + replay history (${ObservedTraffic.all().length + ObservedTraffic.persistedTrafficEntries().length + InvestigationRecorder.timeline().length + SnapshotManager.all().length + StorageWatcher.log().length + EventDebugger.log().length + ReplayHistory.all().length})</button>
            ...
            document.getElementById('ttd-settings-clear-captured').onclick = (e) => {
                if (!confirm('Clear captured Traffic history, the persisted traffic cache, the Recorder timeline, saved Snapshots, the Storage/Cookie Watcher log, the Event Debugger log, and Replay history? Active watches and the Token Vault are untouched (Token Vault has its own Clear button in its panel). This does not touch the page\'s real localStorage, sessionStorage, or cookies.')) return;
                ...
                ReplayHistory.clear();
                e.target.textContent = 'Cleared!';
                setTimeout(() => { e.target.textContent = 'Clear traffic + persisted cache + recorder + snapshots + watch logs + replay history (0)'; }, 1200);
            };
            ...
            const commitSecondaryLogsBudget = () => {
                ...
                StorageWatcher._persistLog();
                EventDebugger._persistLog();
            };
            ...
            const commitSnapshotsBudget = () => {
                const input = document.getElementById('ttd-settings-snapshots-budget');
                if (!input) return;
                Config.snapshotsPersistBudgetBytes = input.value;
                input.value = Config.snapshotsPersistBudgetBytes;
                SnapshotManager._persistNow();
            };
            document.getElementById('ttd-settings-snapshots-budget').onblur = commitSnapshotsBudget;
            document.getElementById('ttd-settings-snapshots-budget').onkeydown = (e) => { if (e.key === 'Enter') { commitSnapshotsBudget(); e.target.blur(); } };

            const commitReplayHistoryBudget = () => {
                const input = document.getElementById('ttd-settings-replay-history-budget');
                if (!input) return;
                Config.replayHistoryBudgetBytes = input.value;
                input.value = Config.replayHistoryBudgetBytes;
                ReplayHistory._persistNow();
            };
            document.getElementById('ttd-settings-replay-history-budget').onblur = commitReplayHistoryBudget;
            document.getElementById('ttd-settings-replay-history-budget').onkeydown = (e) => { if (e.key === 'Enter') { commitReplayHistoryBudget(); e.target.blur(); } };
    ...
    TokenVault.install();
    StorageVault.install();
    StorageWatcher.install();
    EventDebugger.install();
    ReplayHistory.install();
