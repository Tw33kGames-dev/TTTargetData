...
(function () {
    ...
    const Config = {
        ...
        // Snapshots carry a full storage/cookie/globals dump each, materially bigger than the
        // "secondary logs" bucket's log-line-sized entries - gets its own budget rather than
        // sharing that one. Evicted whole-snapshot-at-a-time (oldest first) when over budget,
        // not trimmed internally, since a partially-trimmed snapshot is a worse record of "what
        // did this look like at this moment" than just having fewer complete ones.
        ...
        // Replay/Edit history can carry full response bodies per entry - closer in size to the
        // persisted traffic cache than to a log line, so it gets a comparable budget.
    ...
    const EventDebugger = {
        ...
        // Same asymmetry as DOM Mutation Watcher: a watch holds a live element reference and
        // can't survive a reload, but the log of what it already caught can.
        install() {
    ...
    const StorageWatcher = {
        ...
        // Unlike DOM Mutation Watcher, a storage/cookie watch is just {kind, key} - no live
        // element reference - so it can genuinely auto-resume after a reload rather than only
        // its log surviving. lastValue gets re-read fresh on resume (not restored from before
        // the reload), so resuming doesn't immediately flag a spurious "changed" entry.
        install() {
            this._loadLog();
    ...
    const SnapshotManager = {
        ...
        install() {
            ...
            // Recorder and WebSocketMonitor both got this same safety net; this one was missed -
            // without it, capturing a snapshot and reloading within the 2s debounce window loses
            // it, since the debounced save never gets a chance to fire before the page unloads.
            const flush = () => {
                if (this._saveTimer) { clearTimeout(this._saveTimer); this._saveTimer = null; }
                this._persistNow();
            };
        ...
        // Evicts whole snapshots (oldest first), never trims inside one - a snapshot missing
        // half its localStorage keys is a worse record of "what did this look like" than just
        // keeping fewer complete ones.
        _scheduleSave() {
            if (this._saveTimer) return;
            this._saveTimer = setTimeout(() => {
                this._saveTimer = null;
        ...
        // Overlay restore, not exact-replace: writes every key the snapshot had back onto the
        // live page, but doesn't delete keys that exist now and didn't exist in the snapshot -
        // deleting things not in the snapshot risks wiping legitimate new data that has nothing
        // to do with what was being tested. Globals aren't restored at all - what's captured for
        // them is a string preview (see _dumpGlobals), not the real value, and reassigning
        // arbitrary window properties from a stale preview isn't meaningful. IndexedDB was never
        // part of a snapshot to begin with, so nothing to restore there either.
        restore(id) {
    ...
    const TokenVault = {
        ...
        // Persists across reloads on purpose - this was originally deliberately in-memory-only
        // (real credential material, minimize how long it sits around), reverted to that once,
        // then reconsidered and put back: persistence is genuinely useful for reusing a token
        // across a multi-page-reload flow, and the tradeoff is the user's call to make, not a
        // default this tool should silently override. Immediate (non-debounced) writes, since
        // entries only change on explicit action, not high-frequency events.
        install() {
            const stored = Storage.get(this.KEY, null);
            if (Array.isArray(stored)) {
        ...
        // For a token that didn't come from the page scanner - pasted in from elsewhere, or one
        // you want to test with deliberately (expired, malformed, etc.).
    ...
    // Same purpose as TokenVault, for storage/cookie values: the live Storage/Cookie editors
    // only ever show the *current* value of a key - there's no way to pin a specific value you
    // deliberately want to keep around, independent of whatever's live right now, the way a
    // token can be saved and reused later. This closes that gap. Lighter security framing than
    // TokenVault - these are usually just test data, but a cookie *can* carry session-identifying
    // material, so still worth being a little careful with what ends up here.
    const StorageVault = {
        MAX: 30,
        ...
        // Writes a saved entry back onto the live page - optionally under a different key than
        // it was saved from, so a vault entry can double as a reusable template, not just a
        // restore point for its original key.
    ...
    const InvestigationExport = {
        async gather(categories) {
            ...
            if (categories.domSnapshot) {
                ...
                // Capped at 50 records/store, 500 chars/value - see IndexedDBInspector.snapshotAllRows.
            ...
            // Off by default, unlike everything else here - these are raw auth/session token
            // values. Including them in an export means they end up wherever that export goes
            // (pasted into a chat, saved to a file, etc.) - opt-in only, on purpose.
            if (categories.tokenVault) data.tokenVault = TokenVault.all();
            // Off by default like tokenVault - lower stakes usually (test data), but a cookie
            // value saved here can carry session-identifying material, worth the same caution.
        ...
        async toCSV(categories) {
            ...
            // Recorder entries have a different shape per kind (click/input/network/console/
            // error/pick/etc.) - rather than a fixed column per kind, Timestamp+Kind stay fixed
            // and everything else gets flattened into one Detail column.
            ...
            // One row per message rather than per connection, since a connection's messages
            // are the actually-tabular part - a connection with zero messages still gets one
            ...
            // Summary row per snapshot, not the full nested storage/cookie/globals payload each
            // one carries - that's what the JSON export is for. This is an index, not a dump.
            ...
            if (categories.replayHistory && data.replayHistory && data.replayHistory.length) {
                const rows = [this._csvRow(['Timestamp', 'Method', 'Url', 'RequestBody', 'Ok', 'Status', 'DurationMs', 'ResponseBody', 'Error'])];
        ...
        async briefing(categories) {
            ...
            if (data.domSnapshot && data.domSnapshot.length) {
                ...
                // Built from whatever sources actually showed up rather than a hardcoded list,
                // so this stays accurate if IndexedDB has nothing this time, or something else
                // gets added to domSnapshot later.
    ...
    const PageInspector = {
        ...
        // Scans every accessible stylesheet's rules for class selectors matching a substring -
        // built specifically for CSS-Modules-style class names (e.g. "stockOwned___VshhT"),
        // where the state-driving part is human-readable but the hash suffix means you can't
        // just guess the full name. Cross-origin stylesheets loaded without CORS headers throw
        // on .cssRules access and get skipped - reported as a count, not silently dropped, so
        // a search that comes up empty (or thin) isn't mistaken for "no such class exists."
        // Results are grouped by distinct class name, not one row per CSS rule - a single class
        // routinely gets referenced by many compound/pseudo-state selectors (:hover, :last-child,
        // combined with ancestors, etc.), and a flat per-rule list made "10 matches" look like 10
        // different classes when it was really one class in 10 rules.
    ...
    const IndexedDBInspector = {
        ...
        // Flat {source, key, value} rows for Export, matching the shape domSnapshot already
        // uses for localStorage/sessionStorage/cookies - source is "indexeddb:<db>:<store>" so
        // rows stay traceable back to where they came from. Capped per store (a store can hold
        // thousands of records; this is for spotting/sharing something interesting, not a bulk
        // dump) and each value truncated, same spirit as the persisted traffic cache's body
        // truncation. Best-effort throughout: a database or store that fails to open just gets
    ...
    const Helpers = {
        ...
        // Same look as _primaryBtnStyle(), but sized to its own content instead of width:100% -
        // for a primary button sitting in a flex row next to something else (an input, another
        // button). width:100% inside a flex item inflates its preferred size and squeezes
        // whatever it's next to down to nothing, which is exactly the bug that kept recurring
        // anywhere a primary button was placed beside a text input.
    ...
    const PageInspectorUI = {
        ...
        // Grouped replacement for what used to be one flat 27-item dropdown - the category
        // select picks one of these groups, and the individual pages inside it become tabs
        // above the content instead of dropdown options. Order within each group is the same
        // order those pages used to appear in the old flat list.
        _sectionGroups: [
            {
        ...
        // Only 'websocket' needs a dynamic label (on/off state) - everything else uses the
        // static label from _sectionGroups.
        ...
        // Shared by the section dropdown and any in-panel shortcut button (e.g. "Find a
        // class..." on the Element editor) that jumps to a different section - keeps the
        // per-section cleanup (stopping pick mode, clearing log-change callbacks, stopping a
        // running WS sequence) in one place instead of duplicating it at every jump point.
        _goToSection(name) {
        ...
        _wireElementEditor(pickedEl, t) {
            ...
            // A save "succeeding" with zero visible or reportable effect, even after the fix
            // that made success/revert actually show up, points at a different failure mode
            // than a reverted attribute: the picked node itself may no longer be the one on the
            // page at all. A framework replacing (not just updating) this element leaves pickedEl
            // pointing at an orphaned node - writes to it "succeed" and even the revert-check
            // passes (it's comparing the orphan to itself), while the real, visible element was
            // never touched. Caught up front here so that shows up as a clear message instead of
            ...
            // Any exception here previously aborted the handler with zero visible feedback -
            // this ensures that whatever it is, it becomes a message instead of nothing.
            const safely = (fn) => {
                try {
                    return fn();
                } catch (e) {
            ...
            // "Saved" only means the DOM call itself succeeded - it can't tell you whether a JS
            // framework (React, commonly, on a page like this one) silently re-rendered this
            // element right afterward and overwrote the change, which looks identical to "did
            // nothing" from the outside. This re-checks shortly after the write and says so
            // plainly if the value didn't stick. Not exhaustive - catches a revert within
            // ~300ms, not one triggered later by some unrelated interaction.
            // The panel refresh is deliberately deferred until after this check, not run
            // immediately after save - an immediate refresh was rebuilding the whole panel
            // (including a fresh, empty status line) in the same synchronous tick as report(),
            // so "Saved." never actually got painted before being wiped out.
        ...
        _renderExport() {
            ...
            ['traffic', 'catalog', 'persistedTraffic', 'recorder', 'element', 'pageTiming', 'websocket', 'wsMessageCatalog', 'traceHistory', 'replay', 'replayHistory', 'sandboxRun', 'sandboxHistory', 'domSnapshot', 'eventDebugLog', 'domMutationLog', 'storageWatchLog', 'snapshots', 'tokenVault', 'storageVault'].forEach((key) => {
        ...
        _renderGlobals() {
            ...
            globals.forEach((g) => {
                const row = document.createElement('div');
                row.style.cssText = `padding:6px 8px;font-size:11px;border-bottom:1px solid ${t.rowBorder};word-break:break-all;`;
                const isFn = g.type === 'function';
                row.innerHTML = `
                    <div style="cursor:pointer;">
                        <span style="font-weight:700;">${Helpers._escape(g.key)}</span> <span style="color:${t.cardDesc};">(${Helpers._escape(g.type)})</span><br>
