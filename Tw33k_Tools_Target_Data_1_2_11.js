...
(function () {
    ...
    const TokenVault = {
        ...
        // Saved immediately, not debounced - entries only change on explicit user action (add/
        // remove/clear), not high-frequency events, and losing a manually-saved token to reload
        // timing would be worse than the trivial cost of an immediate write.
        install() {
            const stored = Storage.get(this.KEY, null);
            if (Array.isArray(stored)) {
            ...
            if (categories.snapshots && data.snapshots && data.snapshots.length) {
                ...
                data.snapshots.forEach((s) => {
                    rows.push(this._csvRow([
                        ...
                        (s.data.localStorage || []).length, (s.data.sessionStorage || []).length,
                        (s.data.cookies || []).length, (s.data.globals || []).length
                    ]));
                });
        ...
        _renderExport() {
            ...
            area.innerHTML = `
                ...
                <div style="margin-bottom:8px;">
                    ...
                    <label style="display:flex;align-items:center;gap:6px;font-size:9px;color:${t.statusBad};padding:0 0 4px 24px;">Off by default - includes raw, unredacted auth/session token values. Only check this if you actually want them in whatever this export ends up.</label>
                </div>
                <div style="display:flex;gap:6px;margin-bottom:6px;">
